// ⚠️  VERBATIM COPY — DO NOT EDIT THIS FILE.
//
// The single source of truth is `Pastura/Pastura/LLM/SuspendController.swift`.
// This copy exists because SwiftPM rejects a target source path that escapes
// the package root, so the real file cannot be referenced from here. Every
// line below the sentinel is byte-identical to it, and the nightly guard
// `scripts/check-suspendcontroller-drift.sh` fails if that stops being true.
//
// Why a copy rather than a hand-written stand-in: the real controller carries
// a single-awaiter precondition, an idempotent resume() on .idle, and a
// self-resume race fix. A re-implementation could drop any of those and still
// pass a happy-path relay test — so the gate would report ADR-023 §5.2
// invariant 3 (lost-wakeup safety) as witnessed while testing a weaker object.
//
// To change behaviour: edit the real file, then re-run the copy.
//
// ---8<--- VERBATIM COPY BELOW ---8<---
import Foundation
import os
