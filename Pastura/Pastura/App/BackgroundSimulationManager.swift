...
/// 3. VM calls `scheduleRequest(...)` with activation/expiration callbacks.
/// 4. App is backgrounded; the system activates the task and calls the registered
///    handler, which invokes the `onActivation` callback.
/// 5. VM handles activation: pause sim, reload model on CPU, resume sim.
/// 6. VM periodically calls `updateProgress(...)` so the system doesn't auto-expire.
/// 7. On expiration, completion, or foreground return, VM calls `completeTask(success:)`.
///
...
nonisolated public final class BackgroundSimulationManager: @unchecked Sendable {
  ...
  /// requires user initiation). The callbacks are invoked when the system activates
  /// the task (after app backgrounds) or needs to expire it.
  ///
  /// - Parameters:
  ...
  ///   - onExpiration: Called when the system needs to end the task (resource
  ///     pressure, time limit, user stop). VM should pause and flush state.
  ...
  private func handleTaskLaunch(_ task: BGTask) {
    ...
    continuedTask.expirationHandler = { [weak self] in
      ...
      let onExpiration = self.state.withLock { $0.onExpiration }
      onExpiration?()
    }
