...
| `--moss-ink` | `#3D4030` | 犬の輪郭・完了タイトル・**完了ステータスピルのラベル**（`ResultsView` の完了ピル、`GameHeaderStatus.completed`）。半透明の苔系ウォッシュ上でこの段を使うのは §8 の唯一の例外に当たる — 条件はそちら。ここに無い意味でこのトークンをウォッシュ上に置いているサイトは routing 未正当化（§8 末尾の ⚠️） |
