...
- **Dark mode (§2.9)** — **resolved 2026-07-31, no longer live debt.** All five of ADR-028 § "Rollout"'s removal gates are met and `Info.plist`'s `UIUserInterfaceStyle = Light` is gone, so the app follows the device appearance. The four-slice history, the derivation arms, and the load-bearing "1 unpaired (`headerMetaSubdued`, resolved as fixed in both appearances) vs 0 owing gate 1 an answer" distinction live in [ADR-028](decisions/ADR-028.md) and its INDEX summary — kept there rather than mirrored here, since this bullet went stale on every slice. 残件はゲートを閉じた QA より後に入った 5 件の再確認のみ — `docs/qa/dark-mode-qa.md` の再走リストが正本。`nightPage` の `ViewerPredictionSheet` 適用は実機で決着し（設計上の上下関係が描画時に反転する — ADR-028 § Amendment 2026-08-05）、その実測が Simulation 画面に地の指定が無い欠陥を露出させ、クラスとして列挙した結果 ResultDetail も同状態と判明して 5 件目になった。Manual QA walkthrough: `docs/qa/dark-mode-qa.md`. Tracked in #1274 / #1284 / #1336.
