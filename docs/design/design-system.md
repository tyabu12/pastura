...
**彩度が苔系にならないのは避けられない**: ライトの色味（α 0.10 での合成の G−B ≈ 14.8）を
...
（この2レイヤーは .04 → **.03** / .2 → **.13**、#1378）。

ダークでの沈み込みは α 0.03 で ~0.5 sRGB 段、0.13 で ~2.1 段、0.36 で ~5.8 段
（地の相対輝度 0.0117 に対して下の余地が少ない）。ダークの浮き上がりは主に
「面が地より明るい」＋ヘアラインが担う。
