...
あり、G ≤ 28 / B ≥ 0 なので **G−B は理論上限でも 28**。一方、ライトの色味を再現するには
tint の G−B が苔系そのものの値 **40** 要る（色相を担うのは tint なので、アルファに
よらない）。アルファも解き直した §4.3 の2レイヤーでは、変更前の合成の色味を保つのに
**51**（`.tight`）〜**58**（`.soft`）が要る。どの読み方でも上限 28 に届かない。
