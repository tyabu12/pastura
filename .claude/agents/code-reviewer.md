...
- **Navigation root-stack scope** — no `navigationDestination(item:|isPresented:)` in a view pushed onto a tab stack (sheets exempt); no `router.path` mutation outside `AppRouter` (`rg 'router\.path\s*(=[^=]|\.append|\.remove)'`; the rule is in `App/AppRouter.swift`'s `path` doc comment). → `navigation.md` §"Forbidden inside a tab's stack" / §"AppRouter scope (load-bearing)"
