...
（`--scrim` と同値）を使う。`PasturaOccluderShadows` が正本。
...
**覆いうるすべての地より暗い**ことが要件（§2.9 の `nightPage` #11130F を含む —
`InFlightSimulationIndicator` は root overlay で全画面の上に乗るため）。同じ結論に
先に到達したのが full-bleed の `--scrim`（ADR-028）で、値を共有しているのはそのため。

**彩度が苔系にならないのは避けられない**: 新しい alpha でライトの色味（G−B ≈ 15）を
保つには tint の G−B が 94 必要で、near-black では ~24 が上限。ライト側の影は
ニュートラルになる。

ダークでは実質 ~1 sRGB 段の no-op（地の相対輝度 0.0117 に対して下の余地がない）。
ダークの浮き上がりは影ではなく「面が地より明るい」＋ヘアラインが担う。

上の苔系2レイヤー（`PasturaShadows`）は同じ向きの問題を 1/10 の alpha で抱えるが、
5つの消費者に影響し §4.3 の苔ルール自体の再定義になるため**意図的に据え置き**。
