...
**`--ink-on-wash` の light 値が `--ink-2` と同一なのは偶然ではなく設計。** 別名参照ではなく**値の複製**である。複製が買っているのは dark の独立性ではなく（dark 半分は `PasturaDynamicPalette` で独立に宣言されるので別名でも保たれる）、**逆向きの切り離し** — 将来 `--ink-2` を light 側で再調整したときにこのトークンが黙って追随しないこと。§2.12 の `headerMetaInk` が `metaBaseL3` と同 hex を別役割で複製しているのと同型。壊れていたのは dark だけだったため（4 箇所で 4.413〜4.773:1、うち 2 つは 4.501:1 で「バーちょうど」）light は据え置き、dark だけ `nightInkSecondary` → `nightInkOnWash` に持ち上げた。**したがって light 端末ではこの変更は一切見えない。** **「`inkWashSites` の membership」と「このトークンを塗っているサイト」は別物なので、分けて読むこと。** `DesignTokensTests+InkOnWash` の `inkWashSites` は**同じトークンを文字と半透明ウォッシュの両方に塗っている** *self-wash* だけを持ち、その地は不透明なサーフェスなので worst case として測れる — `PhaseEditorSheet.fieldPill` の thought、`ScenarioBadgeStyle.secondary`、`PhaseTypeLabel` の非 LLM、`ResultsView` の paused ピル。**このトークンを塗るサイトはそれだけではない**: `GameHeaderStatus` の終端状態アーム（paused / cancelled / error）は self-wash ではなく（ウォッシュは `--muted` のまま）、地も `--screen-bg@0.78` × マテリアルなので worst case が書けない。**だからあの membership には入れず、§8 の優先順位 1 に従って nominal 地の専用 arm で測る** — 苔アームが `DesignTokensTests+MossOnWash` で測られているのと同じ形（#1448 batch 4、サイトの pin は `MutedSweepLedgerTests+BatchFour`）。⚠️ **この列挙と fixture を突き合わせるゲートは、インク側には無い。** 苔系の `--moss-ink` には `scripts/check-mossink-wash-membership.py` の set-equality があるが対応物を持たないので、ここの列挙と fixture の membership は**手で**整合させること。不透明な中立塗りは別問題で、§2.6 の Soft+Ink 対が担当する（#1407 と同じ形）。#1408。

**セクションラベルは `--ink-2`。`PasturaSection` を通らない自前の手書きヘッダーもこの表に従う。** かつて `PasturaSection` のヘッダーと手書きヘッダー2つ（`HomeView.scenariosSectionHeader` / `SettingsView+Models.modelsHeader`）は `--muted` で描かれ表と食い違っており、#1485 で揃えた。#1448 が「コードを表に寄せる」と決め — §2.2 が役割の規範的な記述であること、#1298 が既に先例を作っていること、監査で「表のほうが誤り」とする論拠が出なかったこと（導出は `muted-application-audit` §6.3）— #1485 で適用した。**5画面**（ホーム / 設定 / 観察履歴 / ScenarioDetail / GalleryScenarioDetail）にまたがる視覚変更なので ADR-028 gate 4/5 の実機 QA を伴う。掃引対象を数え直すときの正は**呼び出し形**の grep — `rg -l 'PasturaSection\s*[({]' Pastura/Pastura/Views/`。型名だけの grep は `PasturaSectionStyle` の言及も拾って広く出るし、`\(` だけの grep は無題カードの末尾クロージャ形を落とす。**件数はここに写さない**（`muted-application-audit` §6.3 が列挙を持つ。消費側の数は「決定が変わらないまま動く」在庫で、`adr-writing.md` §4 の写し禁止に当たる）— ただし consumer ファイル数 / ヘッダーを実際に描くファイル数 / 画面数は**別の量**で、混ぜると規模を逆向きに誤る。上の5は QA 規模を決める画面数のほうで、§6.3 が3つを分けて持つ。**システム `Form` / `List` の section ヘッダーは、地で決める（#1527）。** これらはどのトークンも読んでおらず、「Pastura は system `Form` の chrome をトークン化するのか」は #1448 では未決のまま #1527 へ切り出された（列挙のレシピと移動 / 据え置きの内訳は `muted-application-audit` §6.3、追跡は台帳 §7 の B6、導出は ADR-028 § Amendment 2026-08-22 (#1527)）。#1527 で決めた規則: **Pastura が `Form` の地を塗っている画面（`.scrollContentBackground(.hidden)` ＋ トークン背景）では、system ヘッダーもこの表に従い `--ink-2`。地がシステム標準のままの `Form` / `List`（Editor の sheet 一式・`LicensesSheet`）では、ヘッダーもシステム標準に任せる。** 鍵は「ホストが `Form` か」ではなく「地を誰が塗っているか」— 暖色の `--ink-2` を冷色の system grouped 地に載せると周囲のシステム chrome の中でそこだけ浮くうえ、その地は §8 の計測 pin に無く、手計算で埋めることは §8 自身が禁じている。§5.8.3 が sheet 内の navigation chrome をシステム標準に残しているのと同じ向きの線引きだが、あの表は toolbar の話なので根拠ではなく類推。#1298 が `ScenarioEditorView` の Personas / Phases だけを揃えたのは隣に `--muted` のカウントが並び、明度がほぼ同じで色相だけ違う組み合わせになっていたためだが、この規則の下では同じ `Form` の Basic Info / Context も同じ地に乗っているので揃える — 「カウント付きだけ」という線引きは規則に吸収された。**件数はここに写さない** — 上と同じ理由で、ADR-028 の対応段落も同じ扱いにしている。
...
- ⚠️ **上の免除は「測った地」にしか及ばない。** ≈ 3.3:1 は `screen-bg` 上の値で、`muted` は出荷している12の地（両外観 6 ずつ）で **2.136〜4.152** に散る。最悪は `--moss-soft` 上の 2.136、ダークで最もバーに近いのが `night-page` 上の 4.152（バーの 8% 下）。**ページ地で正当にアンビエントなラベルが、色付きの塗りの上では大きく下回っていても「§8 公認」に見える** — 地を変えたら測り直すこと。12組は `DesignTokensTests+MutedTranscript` が pin しており、**この行の幅は写しなので** `scripts/check-measurement-transcripts.py` がその pin の最小・最大と突き合わせている（#1488）。**突き合わせの対象外にある写しも野放しではない** — `--census` が写しの**形**（ファイル別の行数・値の種類数・ダイジェスト）を宣言と照合するので、この図をどこかへ写すと CI（`shell-tests`）が落ちる。図そのものを突き合わせるわけではない。落ちたら**貼る前に必ず `--residue` と宣言を突き合わせること** — 腐敗は削除と同じ形で落ち、しかも行数が変わらない腐敗は前置きが「追加」と出る。**縛れるのは pin と同じ3桁表記だけで、桁を落とした言い換えは素通りする**。残る穴と届く範囲はチェッカーの module docstring、導出は ADR-028 § Amendment 2026-08-20 (#1496)。残りサイトのアプリ全体掃引は #1448（完了。トークンを読まない system `Form` / `List` のヘッダーは #1527 で「地で決める」と決着 — §2.2）
- ⚠️ **不透明な12の地でも足りない — 半透明ウォッシュ上に「測っていない地」がもう一段ある。** `muted` テキストが実際に載るウォッシュは3種類（自分自身のウォッシュ、苔系ウォッシュ、`ReportSheet` チップの `rule@0.45`）で、いずれも上の12組のどれとも一致しない。シート地は Pastura トークンではないので、地を1つ決め打ちせず**全12地に対して量化**している。**「新しい最悪」ではなく「§8 が測っていない地」** — どれも `--moss-soft` 上の 2.136 を下回らないので12組の幅がトークンを依然として囲む。この順序そのものを `compositedGroundsStayAboveTheOpaqueWorstCase` が pin する。⚠️ **「未測定」は「違反」ではない。** U は「§8 を根拠に引用できない」という意味であって、repoint の要否は上のバレット3の役割判定（一覧キャプション・脚注・アンビエントか）で決める。役割がアンビエントなら、測り直した比が sub-AA でもバレット3のまま**据え置く** — #1448 の掃引が最後まで据え置いたのがその実例で、**§8 のティアで**据え置いたのが `ResultsView` の `.pending` ピル、`SimulationView` のスクリム字幕、`ReportSheet` の ID チップ。`ActiveModelChip` のシェブロンとステータスドットも据え置きだが、**あちらは非テキストなので根拠が §8 ではなく 1.4.11** — 同じ「据え置き」でも規則の出どころが違うので、まとめて数えないこと。サイトごとの裁定は `muted-application-audit` §6.4。ここを「未測定だから上げる」と読むと、§1 の「静謐・観察」を体現するために意図的に置いた階層を機械的に潰すことになり、出荷後に戻すのは難しい。**比を合成で出すときは fixture の `composite` / `contrastRatio` を使うこと**（チャンネルを 0–255 に量子化する自作スクリプトはずれる）。**サイトごとの比はここに写さない** — 正本は fixture が実行時に計算し、その転記が `muted-application-audit` §3.2（導出は ADR-028 § Amendment 2026-08-15）
...
- ⚠️ **静的な比が出ない地でも routing は決まる。ただし「測れない」を最初の答えにしないこと。** マテリアル（`.ultraThinMaterial` / `.regularMaterial`）、半透明の上にさらに半透明が重なった層、Pastura トークンを持たないシステムのシート地 — この 3 種は真の地が実行時に決まる。**それでも**地を所有するファミリは読めるので、置き換え先の既定は上のバレットのまま: 中立なら `--ink-2`、インク系の半透明ウォッシュなら `--ink-on-wash`、苔系なら `--moss-on-wash`。根拠の付け方には**優先順位がある**:
  1. **nominal 地で実測する（既定）。** 不透明な下限が 1 つでも書けるなら、それを地として fixture で測り、その行に *nominal*（worst case ではない）と注記する。`GameHeader` のバーは `--screen-bg@0.78` をマテリアルの上に敷くので下限が書け、`DesignTokensTests+MossOnWash` の `GameHeaderStatus.active` 行が実際にこの形で測られている。**同じピルの終端状態アームも同じ形で測る**（#1448 batch 4）。nominal な比は bound ではないので、埋めるのは実機 QA。
  2. **それも書けないときに限り、方向で論証する。** 置き換え先が light で `--muted` より暗く、dark で `nightMuted` より明るいこと。⚠️ **これだけでは足りない** — 「暗くすれば比が上がる」が**保証されるのは、地が `--muted` より明るいときだけ**。地がそこより暗くなると、**2つの前景の中間のどこかで符号が反転し**、そこから下では repoint がコントラストを**下げる**（`--muted` のほうが地から遠くなるため）。⚠️ **反転が起きるのは「地が両方の前景より暗いとき」ではなく、その手前の帯である** — 「置き換え先より暗くはないから安全」という読み方は成立しない。反例は `DesignTokensTests+MutedDirection` の `directionArgumentInvertsOnAGroundDarkerThanMuted` が構成して pin しており、その地はまさにこの帯（`--muted` より暗く `--ink-2` より明るい）にある。比はここに写さない。だから方向論証は「地の明度が light では `--muted` より上、dark では `nightMuted` より下だと**言い切れる**場合」に限って使うこと — 帯に入りうるなら使えない。言い切れないなら 1 に戻るか、repoint 自体を見送る。使ったサイトは **why-comment に「方向論証」と明記し、比は書かない**（無い）。ガードは fixture ではなく `MutedSweepLedgerTests` の**シンボル固定**。先例: #1448 batch 1 の `DLCompleteOverlay`（マテリアルのみで下限が無い）、batch 3 の `ScoreboardSheet` の 2 行（システムのシート地）。

  ⚠️ **どちらの経路も「測れないから上げてよい」ではない** — repoint の要否は先に上の役割判定（アンビエントか、A1〜A5 か）で決めること。ここが効くのは「上げると決めた後、どのトークンへ、どんな根拠で」だけで、役割がアンビエントなら測れない地でも据え置く
- **§2.7 の**値**を使った塗りでも、状態で出入りするものは §2.3 のウォッシュではない。** `hover` / `pressed` / `selected`（moss 6/12/18%）を**要素の状態に応じて出したり消したりする**塗りは、§2.7 自身が「下地のサーフェス色を問わない」と書くとおり状態を示す薄膜であって、要素の地を定義しない。したがって既定の「半透明のウォッシュ上なら `*-on-wash`」は**そこには及ばない** — 地はオーバーレイの下のサーフェスのままで、文字トークンはその地で選ぶ。**独立した根拠**: ウォッシュと読むと同じ 1 行の同じ文字が選択時 `--moss-on-wash` / 非選択時 `--ink-2` と**実行時に入れ替わる**ことになり、routing として成立しない。実例: `ModelRow` の選択行は `moss@0.06`（=`hover` と同値）の下が `--bubble-bg` で、タイトルは `--ink`、メタは `--ink-2`（#1448 batch 4）。⚠️ **これはトークン単位の判定ではない。** 同じ §2.7 トークンを**常設**の装飾ウォッシュとして塗るサイトは既定どおりウォッシュで、`--moss-on-wash` が要る — `GalleryCatalogRow.categoryChip` が `Color.selected` をそう使っており、#1327 が実測して `mossWashSites` に pin している。**見分けるのは塗りの出入りであってトークン名ではない**。ただし**比はオーバーレイ込みで測る**こと（薄膜の分だけ下がるので、素の地の値を流用しない）。fixture の `composite` を 2 段重ねれば出る
