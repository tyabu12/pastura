...
| 5 | iOS consumption switch + code-merge | ⬜ not started | the remaining integration · when writing the Swift `LLMBackend` adapter, confirm whether `knownTurnMarkers`' Kotlin interface default crosses K/N — by compiling the adapter, since a `PasturaShared.h` read alone does not settle it (see that member's KDoc) · **`ScenarioValidationMessage.render()` is en-only**: its 53 literals all have `ja` translations in `Localizable.xcstrings`, so consuming the Kotlin engine before that becomes an `expect`/`actual` leaf ships English validation errors to `ja` users — no compiler or test signal, both sides stay internally consistent ([#1464](https://github.com/tyabu12/pastura/issues/1464)) |
