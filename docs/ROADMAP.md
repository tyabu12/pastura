...
- **Dark mode (§2.9)** — post-v1.0 design debt, **not Phase 3 scope**, so it is not blocked by the Phase 3 gate. The pairing mechanism landed (ADR-028) and **67 light tokens** now resolve trait-based against their `night*` counterparts (the *light* alias is the dynamic side; `night*` stays static). **Three of the five removal gates are met**: gate 1 — every light token now has a designed dark value or a recorded fixed decision, closed by slice 4 — plus raw-SwiftUI-colour tokenization (#1283 — gate 3; also added `inkOnAccent` and retired `Color.accentColor`) and the `nightSurface` double-mapping, resolved by deleting it (#1282 — gate 2). What remains is **real-device QA only**: gate 4 (dark QA across Home / ScenarioDetail / Demo / Simulation / Results / Settings) and gate 5 (the dark share-card path, unreachable while the lock holds and unobservable by any test), before `Info.plist`'s `UIUserInterfaceStyle = Light` can be removed. **Exactly one** light token is unpaired — `headerMetaSubdued`, resolved as fixed in both appearances, which gate 1 accepts as an equal answer — so "1 unpaired" and "0 owing an answer" are different counts and a statement of one must not be read as the other. All four slices shipped: #1282 **slice 1** (§2.6 alert family + §2.7 interactive states, 18 tokens)、#1313 **slice 2** (§2.4 meta L1–L4 + §2.12 GameHeader slots, 15 tokens)、#1319 **slice 3** (the 17-colour §2.5 avatar palette) and #1325 **slice 4** (the §2.1/§2.3/§2.8 remainder plus `inkOnAccent`, 10 tokens). Sequence by **screen**, not token count: Home was already near-complete because its dominant tokens are all in the original paired 8; the alert-family / interactive-state surfaces (Settings, download progress) carried the bulk of the gap, which is why they went first. Avatar landed in slice 3, which also had to pin `SheepAvatar` to an explicit appearance — pairing §2.5 un-guarded the share-card export one level below `HighlightCardPalette`. Slice 4 discharged the tripwire slice 2 armed on `promoBackground`, re-measuring the §2.4 ladder against the real render ground. Tracked in #1274.
