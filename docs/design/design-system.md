...
| トークン | Hex | 用途 |
|---------|-----|------|
| `--moss-on-wash` | `#535D40` | 半透明の苔系ウォッシュに乗るアクセント文字（バッジ・チップ・ステータスピル）。ステータスピルは**実行中系のアームのみ**（完了アームは上の `--moss-ink`、§8 の例外）。ダーク対は §2.9 の `nightMossOnWash`（#1327 で追加。下の「slice 4」には**含まれない**） |
