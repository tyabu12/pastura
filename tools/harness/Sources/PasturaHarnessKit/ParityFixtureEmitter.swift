...
package enum ParityFixtureEmitter {
  ...
  ///
  /// **That gate is not wired into CI yet.** It lands in slice 1b alongside the
  /// Kotlin consumer — wiring it here would gate a fixture nothing reads. Until
  /// then the golden can go stale between merges; 1b's first act is to
  /// regenerate and add the `harness-build` step next to `emit-golden --check`.
