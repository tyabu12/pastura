...
- **Dark mode (§2.9)** — post-v1.0 design debt, **not Phase 3 scope**, so it is not blocked by the Phase 3 gate. The pairing mechanism landed (ADR-028): 8 light tokens now resolve trait-based against their `night*` counterparts (the *light* alias is the dynamic side; `night*` stays static). What remains is staged — design dark values for the other 59 unpaired light tokens (visual sign-off per token), resolve the `nightSurface` double-mapping, tokenize the remaining raw-SwiftUI-colour sites, then real-device QA before `Info.plist`'s `UIUserInterfaceStyle = Light` can be removed. Sequence by **screen**, not token count: Home is already near-complete because its dominant tokens are all in the paired 8, while the alert-family / interactive-state / avatar / meta-L surfaces (Settings, download progress, editor) carry the bulk of the gap. Tracked in #1274.
