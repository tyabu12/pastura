...
- **i18n leak detection — Tier 1 blind spots** — Tier 1 SwiftLint (`unwrapped_user_facing_string`) only fires on the empirical 10 view-model property names (`errorMessage` / `validationErrors` / `nameError` / `descriptionError` / `conditionError` / `promptError` / `outcomeAlert` / `deepLinkError` / `alertMessage` / `toastMessage`). It cannot catch (a) NEW view-model properties not yet in the regex — flag and request the regex be extended; (b) helper functions returning display-bound `String` displayed via `Text(_:)` (e.g. `phaseTypeDescription` pattern from PR #288 d446fd9 — 10 unwrapped sites slipped past the original sweep); (c) computed properties returning raw English literals. For PRs that add new view-model surface, new helpers returning user-facing `String`, or new `Text(varName)` of computed values, recommend running `python3 scripts/check_i18n_potential_keys.py` (Tier 2) locally and triaging the diff. Full architecture in `docs/i18n/leak-detection.md`.
