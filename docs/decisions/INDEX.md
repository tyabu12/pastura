...
Dark-mode token pairing mechanism — `PasturaDynamicColor` wraps a light/dark `PasturaColorValue` pair in a `UIColor(dynamicProvider:)` closure branching on `traitCollection.userInterfaceStyle`, and the paired `Color.*` aliases point at it, so every existing consumer adapts with **no callsite edit** (520 `Color.<name>` references / 218 files at `15f8084`). **Eight pairs, not nine**: §2.9's table maps both `nightSurface` and `nightBubble` onto `bubbleBackground`, so `nightSurface` (zero consumers) has no light partner — dark wants a background/surface/bubble three-step where light has a two-step, and resolving it (add a light `surface` token / **delete `nightSurface`**, the leading candidate per §573's 追加より削除 / context-pairing = rejected) is deferred to the palette-completion follow-up. The **light** alias is the dynamic side; `night*` stays static so an explicit dark request stays expressible. **59** of 67 light tokens still have no dark counterpart (84 aliases − 9 night − 4 time-of-day − 4 chart, minus the 8 paired), so `UIUserInterfaceStyle = Light` **stays** — the lock is what prevents a half-dark surface, and removing it is gated on 5 conditions (59 values designed, `nightSurface` resolved, 30 raw-SwiftUI-color sites in 13 files tokenized, real-device QA on 6 screens, dark share-card re-verified on device). Verified via three deliberately-separated channels: `UIColor.resolvedColor(with:)` (mechanism — deterministic, skips `Color.Resolved`'s documented linear-light lossiness), `Color.resolve(in:)` (SwiftUI plumbing, i.e. that the *aliases* are dynamic) + a **negative control** of 5 unpaired aliases that must NOT change, and one rendered-app check with the lock temporarily lifted (Home only, iOS 26.5 **simulator** — trait routing reaches rendering; explicitly NOT dark QA). That check also found the unpaired damage is **concentrated, not uniform** — Home is already near-complete because its dominant tokens are all in the paired eight, so sequence the follow-up by screen. `HighlightShareCard.Palette` migrated to raw `PasturaPalette` values in the same change: it built **both** families from `Color.*` aliases, so rewiring would have collapsed light and dark into one with **nothing** to catch it (token tests assert `PasturaPalette`; ADR-009 rules out snapshots; the lock hides the symptom until whoever removes it). Isolation: the provider captures only copied `Sendable` values, never `self` (swift-isolation.md Patterns 6–7, no diagnostic); `PasturaDynamicColor` is deliberately **not** `Equatable` (a synthesized `==` needs `PasturaColorValue`'s MainActor-isolated conformance from a nonisolated context — Pattern 5, under `InferIsolatedConformances`); `PasturaDynamicPalette` is deliberately **not** `nonisolated` (its statics read MainActor `PasturaPalette`). Generalizable lesson: a compile-only probe that constructed the tokens **inline** passed while the real code reading them from `PasturaPalette`'s MainActor statics failed — a probe must replicate the dependency shape, not just the API shape. Asset-catalog color sets rejected because they blind two live gates (`check_design_tokens_css.py`'s hex→`tokens.css` mirror, and `DesignTokensTests`' sRGB-component assertions) (Status: Accepted; #1274)
