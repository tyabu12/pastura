...
**`--ink-on-wash` の light 値が `--ink-2` と同一なのは偶然ではなく設計。** 別名参照ではなく**値の複製**である。複製が買っているのは dark の独立性ではなく（dark 半分は `PasturaDynamicPalette` で独立に宣言されるので別名でも保たれる）、**逆向きの切り離し** — 将来 `--ink-2` を light 側で再調整したときにこのトークンが黙って追随しないこと。§2.12 の `headerMetaInk` が `metaBaseL3` と同 hex を別役割で複製しているのと同型。壊れていたのは dark だけだったため（4 箇所で 4.413〜4.773:1、うち 2 つは 4.501:1 で「バーちょうど」）light は据え置き、dark だけ `nightInkSecondary` → `nightInkOnWash` に持ち上げた。**したがって light 端末ではこの変更は一切見えない。** **「計測した集合」と「適用先の集合」は別物なので、分けて読むこと。** **計測**の正本は `DesignTokensTests+InkOnWash` で、その対象は「同じトークンを文字と半透明ウォッシュの両方に塗っている」**4 箇所（*self-wash*）** — `PhaseEditorSheet.fieldPill` の thought、`ScenarioBadgeStyle.secondary`、`PhaseTypeLabel` の非 LLM、`ResultsView` の paused ピル。**適用先**は現在**5 箇所**で、5 つ目は `GameHeaderStatus` の終端状態アーム（paused / cancelled / error）。こちらは self-wash ではなく（ウォッシュは `--muted` のまま）、しかも地がマテリアル越しなので比が出ない — §8 の「測れない地でも routing は決まる」バレットに従う**方向論証**で着地しており、pin は fixture ではなく `MutedSweepLedgerTests+BatchFour` のシンボル固定（#1448 batch 4）。**だから `inkWashSites` は 4 のままが正しい** — 5 に増やすと合成できない地を fixture に持ち込むことになる。⚠️ **この 2 つの数を突き合わせるゲートは、インク側には無い。** 苔系の `--moss-ink` には `scripts/check-mossink-wash-membership.py` の set-equality があるが対応物を持たないので、ここの列挙と fixture の membership は**手で**整合させること。不透明な中立塗りは別問題で、§2.6 の Soft+Ink 対が担当する（#1407 と同じ形）。#1408。
...
- ⚠️ **不透明な12の地でも足りない — 半透明ウォッシュ上に「測っていない地」がもう一段ある。** `muted` テキストが実際に載るウォッシュは3種類（自分自身のウォッシュ、苔系ウォッシュ、`ReportSheet` チップの `rule@0.45`）で、いずれも上の12組のどれとも一致しない。シート地は Pastura トークンではないので、地を1つ決め打ちせず**全12地に対して量化**している。**「新しい最悪」ではなく「§8 が測っていない地」** — どれも `--moss-soft` 上の 2.136 を下回らないので12組の幅がトークンを依然として囲む。この順序そのものを `compositedGroundsStayAboveTheOpaqueWorstCase` が pin する。⚠️ **「未測定」は「違反」ではない。** U は「§8 を根拠に引用できない」という意味であって、repoint の要否は上のバレット3の役割判定（一覧キャプション・脚注・アンビエントか）で決める。役割がアンビエントなら、測り直した比が sub-AA でもバレット3のまま**据え置く** — #1448 の掃引が最後まで据え置いた 5 行がその実例で、`ResultsView` の `.pending` ピル、`ActiveModelChip` の 2 サイト（こちらは非テキストなので 1.4.11 側）、`SimulationView` のスクリム字幕、`ReportSheet` の ID チップ、いずれも意図的に quiet なまま残してある。5 行それぞれの裁定は `muted-application-audit` §6.4。ここを「未測定だから上げる」と読むと、§1 の「静謐・観察」を体現するために意図的に置いた階層を機械的に潰すことになり、出荷後に戻すのは難しい。**比を合成で出すときは fixture の `composite` / `contrastRatio` を使うこと**（チャンネルを 0–255 に量子化する自作スクリプトはずれる）。**サイトごとの比はここに写さない** — 正本は fixture が実行時に計算し、その転記が `muted-application-audit` §3.2（導出は ADR-028 § Amendment 2026-08-15）
...
- ⚠️ **測れない地でも routing は決まる。「測れない」は「決められない」ではない。** マテリアル（`.ultraThinMaterial` / `.regularMaterial`）、半透明の上にさらに半透明が重なった層、Pastura トークンを持たないシステムのシート地 — この 3 種は静的な比が存在しないので、置き換え先を「測って選ぶ」ことができない。それでも**地を所有するファミリは読める**ので、上の既定はそのまま適用する: 中立なら `--ink-2`、インク系の半透明ウォッシュなら `--ink-on-wash`、苔系なら `--moss-on-wash`。比の代わりに**方向**で論証すること — 置き換え先が light で `--muted` より暗く、dark で `nightMuted` より明るい、つまり**どちらの外観でもコントラストは上がる**という主張だけが立つ。計測より弱い主張なので、**why-comment に「方向論証」と明記し、比は書かない**（書けない）。ガードは fixture ではなく `MutedSweepLedgerTests` の**シンボル固定**が担う。先例は #1448 の 3 バッチ: batch 1 の `DLCompleteOverlay`（マテリアル）、batch 3 の `ScoreboardSheet` の 2 行（シート地）、batch 4 の `GameHeaderStatus` の終端状態アーム（`--muted` ウォッシュがさらにマテリアルの上に乗る二重の半透明）。⚠️ **これは「測れないから上げてよい」ではない** — repoint の要否は先に上の役割判定（アンビエントか、A1〜A5 か）で決めること。方向論証が効くのは「上げると決めた後、どのトークンへ」だけで、役割がアンビエントなら測れない地でも据え置く
- **§2.7 の対話状態オーバーレイは §2.3 のウォッシュではない。** `hover` / `pressed` / `selected`（moss 6/12/18%）は §2.7 自身が「下地のサーフェス色を問わない」と書くとおり**状態を示す薄膜**であって、要素の地を定義しない。したがって上の既定の「半透明のウォッシュ上なら `*-on-wash`」は**ここには及ばない** — 地はオーバーレイの下のサーフェスのままで、文字トークンはその地で選ぶ。ただし**比はオーバーレイ込みで測る**こと（薄膜の分だけ下がるので、素の地の値を流用しない）。fixture の `composite` を 2 段重ねれば出る。実例: `ModelRow` の選択行は `moss@0.06`（=`hover` と同値）の下が `--bubble-bg` で、タイトルは `--ink`、メタは `--ink-2`（#1448 batch 4）。**見分け方は塗りの出入り** — アルファが §2.7 の 3 値のいずれかで、要素の**状態**に応じて現れたり消えたりするならオーバーレイ。常に塗られていて要素の外観そのものを作っているならウォッシュで、既定どおり `*-on-wash` が要る
