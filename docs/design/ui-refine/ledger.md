...
| UR-003 | 2026-06-24 | L5 | 07-results | `"%lld records"` not plural-aware (renders "1 records") | done | Fixed: callsite → `Text("\(count) records")` (Int-interp plural); en `variations.plural` one/other + ja single-form; `extractionState=manual` (key invisible to the CLI extract). Added i18n.md § Plurals + a catalog-structure test. |
