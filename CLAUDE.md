...
- **Logger privacy:** OSLog redacts `String` / `Substring` / `Error` interpolations as `<private>` in TestFlight / Release. Always annotate string-like interpolations in `.error` / `.warning` / `.info` / `.notice` / `.fault` calls with `privacy: .public` (numeric and `Bool` interpolations default to `.public` and don't need annotation; `.debug` is production-suppressed and can be left bare). Diagnostic strings — function names, file paths, type descriptions, error messages — do not contain PII and are safely `.public`. PII candidates (user-entered scenario text, agent outputs) should NOT be Logger-interpolated at all; route through `TurnRecord` persistence instead. _Narrow exception:_ when the same data is already persisted on-device (e.g. via `TurnRecord.rawOutput` per ADR-001), Logger interpolation with `.public` is acceptable for diagnostic purposes provided the callsite carries an inline comment naming the persistence equivalence — see `LLMCaller.logParseFailure` for the pattern. Public-API parameters that could later receive user-controlled strings (e.g. a `title:` argument on a public method) are an exception: prefer logging a compile-time constant identifier with `.public` over annotating the parameter — see `BackgroundSimulationManager.scheduleRequest` for the pattern.
