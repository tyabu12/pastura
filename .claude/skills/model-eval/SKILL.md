...
| `reason=no-grammar` | the turn had no `output:` schema, so `createSampler` returned before the DRY builder. **Zero in every battery cell today**, and structurally so: each Engine phase that reaches the LLM supplies a schema — author-declared, or Engine-fixed as on `narrate` (`{ commentary }`) and `reflect` (`{ note }`). The schema-less shape is the `LLMService` convenience overload (`generate(system:user:)`), which no phase handler calls. A non-zero count therefore means a *new phase or handler* skipped the schema, not a sampler defect |
