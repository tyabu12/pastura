...
/// Catalog-structure assertion for the Browse hidden-installed footer plural
/// (`"%lld installed scenarios hidden"`, #1565) — the sibling of
/// `RecordsCountPluralTests`. A change-detector test (`.claude/rules/view-testing.md`
...
struct HiddenInstalledCountPluralTests {
  ...
  /// The full `"%lld installed scenarios hidden"` catalog entry.
  private func entry() throws -> [String: Any] {
    let data = try Data(contentsOf: Self.catalogURL())
    let json = try JSONSerialization.jsonObject(with: data) as? [String: Any]
    let strings = try #require(json?["strings"] as? [String: Any])
    return try #require(strings["%lld installed scenarios hidden"] as? [String: Any])
  }

  /// The `"%lld installed scenarios hidden"` entry's `localizations` object.
  ...
  @Test func englishHasPluralVariations() throws {
    ...
    #expect(oneValue == "%lld installed scenario hidden")
    #expect(oneState == "translated")

    let (otherValue, otherState) = try stringUnit(plural["other"] as? [String: Any])
    #expect(otherValue == "%lld installed scenarios hidden")
  ...
  @Test func japaneseStaysSingleForm() throws {
    ...
    #expect(value == "インストール済みのシナリオ %lld 件を非表示")
  ...
  @Test func englishPluralFiresAtRuntime() throws {
    ...
    // catalog regression. This is the UR-003 class of regression
    // — n=1 rendering "1 installed scenarios hidden". Not a tautology (asserts hardcoded literals)
    ...
    #expect(
      String(localized: "\(1) installed scenarios hidden", bundle: bundle, locale: en)
        == "1 installed scenario hidden")
    #expect(
      String(localized: "\(2) installed scenarios hidden", bundle: bundle, locale: en)
        == "2 installed scenarios hidden")
    ...
    #expect(
      String(localized: "\(0) installed scenarios hidden", bundle: bundle, locale: en)
        == "0 installed scenarios hidden")
  }

  @Test func keyIsManuallyManaged() throws {
    // `extractionState: "manual"` is load-bearing. The callsite is
    // `Text("\(count) installed scenarios hidden")` — the COMPILER extracts key "%lld installed scenarios hidden"
