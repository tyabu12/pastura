...
3. **LLM backend: the existing `LlamaCppService`, unchanged.** The pinned
   ...
   `.macOS(.v15)` floor. Re-verified against the b10327 artifact at the pin
   bump (#1487); the same three facts held at the original b8694 pin, so this
   is a re-measurement rather than a change. The harness passes an absolute
   GGUF path via `--model` — `LlamaCppService.init` already takes
