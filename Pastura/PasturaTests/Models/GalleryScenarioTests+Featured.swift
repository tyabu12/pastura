...
extension GalleryScenarioTests {
  ...
  // MARK: - featured (optional, curator-assigned pin rank, ADR-025)

  @Test func decodeWithFeatured() throws {
    let json = """
      {
        "id": "future_v1",
        "title": "Future",
    ...
    #expect(scenario.featured == 1)
  }

  /// Forward-compat: a feed predating the `featured` key (or an older app
  /// reading a newer feed) must decode with `featured == nil` — the entry
  /// falls through to the default `added_at`-descending recency order. Same
  /// posture as `agentCount` / `rounds` / `phases` / `language` /
  /// `minEngineVersion`.
  @Test func decodeWithoutFeaturedYieldsNil() throws {
    let json = """
      {
        "id": "legacy_v1",
        "title": "Legacy",
