...
- **User-facing String literals:** Any new user-facing English `String` literal — `Text("...")`, alert / toast / `errorMessage` assignments, accessibility labels — must be wrapped in `String(localized: "...")` so it lands in `Localizable.xcstrings` and gets a `ja` translation. Three independent tiers enforce this, each closing a different gap (full architecture: `docs/i18n/leak-detection.md`). **Tier 1 (edit-time tripwire):** SwiftLint custom rule `unwrapped_user_facing_string` in `.swiftlint.yml` fires on the empirical 10 view-model property assignments (`errorMessage = "..."` shape) — pre-commit hook + `xcb build` / `test` block on it. **Tier 2 (developer audit):** `python3 scripts/check_i18n_potential_keys.py` runs `xcstringstool extract --all-potential-swift-keys` over `Pastura/Pastura/**` (Engine excluded per ADR-010 §4) and surfaces unwrapped literals Tier 1's regex cannot model — helper-returned `String` displayed via `Text(_:)`, computed properties returning English literals. Not CI-gated; invoke locally before opening PRs that add new view-model surface or display-bound helpers. **Tier 3 (CI coverage gate):** `scripts/check_localization_coverage.py` fails CI when any catalog key has a missing or empty `ja` value (already-wrapped keys without translations).
