...
final class TabCoordinator {
  ...
  ///
  /// There is no mid-run leave-guard here: focus mode (ADR-017) hides the tab
  /// bar while a simulation is on top, so a cross-tab switch cannot be triggered
  /// during a run. The #673 defer path was removed with #646; re-selection still
  /// pops-to-root.
