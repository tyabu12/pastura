...
/// Guards `PasturaShadows` (§4.3's general card-elevation recipe) against the
/// same two failure modes as its sibling `PasturaOccluderShadowsTests`, for the
/// same reasons — the two families now share a tint and a requirement, and
...
/// than #0B0C0A would not be picked up. Two notes on its membership:
/// `screenBackground` and `nightBackground` are the binding grounds — three of
/// the four consumers float directly on one of them. `nightBubble` covers the
/// two nested cases (`ResultsView`'s timeline card, and `ScenarioArtTile`'s
/// badge inside a `GalleryCatalogRow`); the art tile's own `moss`-wash fill is
...
/// `nightPage` is asserted as deliberate margin, not because a consumer sits
/// on it.
///
...
struct PasturaShadowsTests {
  ...
  @Test func tintIsDarkerThanEveryGroundItCanCover() {
    ...
    // `entries` IS `all`, so that case leaves the count at 2 and passes.
