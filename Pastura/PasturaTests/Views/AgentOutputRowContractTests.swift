...
struct AgentOutputRowContractTests {
  ...
  // MARK: - shouldRevealThoughtSection — primary-reveal completion gate
  //
  ...
  // driven gate so all four call sites share the same two-phase reveal:
  // primary first, then chevron + thought body.
  ...
  @Test func shouldRevealThoughtSectionUsesStreamingPrimaryLength() {
    ...
    // committed primary is longer or shorter.
    let row = AgentOutputRow(
      agent: "Alice",
      output: TurnOutput(fields: [:]),
      phaseType: .speakAll,
  ...
  @Test func shouldRevealThoughtSectionTrueWhenPrimaryNil() {
    // Edge: rows with no primary (rare — fall-through path for outputs
    // that lack the canonical phase field). `primaryText?.count ?? 0 == 0`
    // → gate satisfied at visibleChars=0. Chevron then renders from
    // frame 1, matching pre-fix behavior for primary-less rows.
    let row = AgentOutputRow(
      agent: "Alice",
