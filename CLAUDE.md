...
- ⚠️ **`Color.*` means "the device's appearance"** for every token registered in `PasturaDynamicPalette` — a fixed-appearance consumer (`ImageRenderer` export) must read `PasturaPalette.<token>.color` directly, and so must any **component** it draws (ADR-028). Deliberately stated without a count: the number moves every gate-1 slice.
