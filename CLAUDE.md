...
Titles are kept byte-identical to INDEX's `## ADR-NNN — <title>` headings. **Nothing prompts an author to append here** — add a new ADR by hand (`adr-writing.md` §4). A `/consistency-audit` run flags the omission afterwards as `adr_roster_drift`; that is after-the-fact and manual, not a pre-merge gate. ADR-006 is reserved-unwritten — see its row above.
