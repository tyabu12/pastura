...
- **Stage 4** (parity harness): ✅ complete (closed 2026-08-30, operator call on
  ...
  **accepted permanently; neither engine changes** ([ADR-023 §15](decisions/ADR-023.md)). The
  ...
  remains.
- **Stage 5** (iOS switch + code-merge): ✅ complete (closed 2026-09-07,
  [#1685](https://github.com/tyabu12/pastura/issues/1685)) — the Kotlin engine is the shipping
  engine for every fresh run and Decision 6 has fired; the H7 probe, the S5-4 opt-in switch and
  the Stage-2 gate spike are all deleted. Two residues, both accepted: the YAML-less run branch
  kept as a test seam ([#1687](https://github.com/tyabu12/pastura/issues/1687)) and the absence
  ...
  switch is opt-in and **S5-5 flips the default**. **S5-5 landed 2026-09-07**
  ([#1685](https://github.com/tyabu12/pastura/issues/1685)): the default is flipped,
  `TurnOutput.rawText` is ported, the H7 probe and switch surface are deleted across eleven
  sites, and the gate spike is retired with the B′ guard re-homed to
  `scripts/kmp/check-b-prime-isolation.sh` and the `macosArm64` parity rung untouched.
  Decision 6 fired and ADR-004 §9.7 closed (ADR-004 §13). See
  [ADR-023](decisions/ADR-023.md) §6 Stage 5 and §16.
