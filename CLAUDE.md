...
| `docs/decisions/ADR-027.md`           | Generic `pairwise_payoff` scoring logic — payoff matrix moves from Swift to scenario YAML (`payoff: [{when, points}]`), unblocking ja `choose` options. `prisonersDilemma` kept **permanently** as a shim (Copy & Edit put its YAML on user devices; local records are ADR-020 D5-only ⇒ no migration path); legacy shim table keeps all 4 rows — `default:`'s 1,1 is the game rule, not a catch-all. ⚠️ ADR-022's gate misses `==` predicates: 2 sites need hand-updating on any `ScoreCalcLogic` addition (Status: Accepted; #1151) |
