...
extension JSONResponseParserTests {
  ...
  /// Pins the Swift/Kotlin multi-object asymmetry: Swift's schema-guarded
  /// salvage (#907) accepts the first object, Kotlin fails the parse. This is
  /// the concrete re-arm candidate for the ADR-023 Stage-4 divergent
  /// fixture's lost structural arm (see `docs/kmp-migration-status.md`
  /// § Stage 4). Not a bug on either side — a divergence that must not
  /// change silently.
  @Test func salvagesFirstObjectFromMultiObjectResponseWhenSchemaGuardPasses() throws {
    let (output, kind) = try parser.parse(
      #"{"statement": "hello", "inner_thought": "thinking"}{"stray": 1}"#,
      expectedKeys: ["statement", "inner_thought"])
    #expect(output.fields["statement"] == "hello")
    #expect(output.fields["inner_thought"] == "thinking")
    #expect(kind == "multi_object_salvage")
  }
}
