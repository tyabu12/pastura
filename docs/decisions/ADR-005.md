...
input validation, defense-in-depth output filtering, Shared Scenario reporting,
...
- **Shared Scenarios** — read-only curated gallery (see `docs/gallery/README.md`).
...
| Shared Scenarios has no reporting mechanism or reviewer identity disclosure | §6 |
...
Shared Scenarios is **curated-only** for Phase 2. All scenarios originate from the
...
consent. ADR-006 (Accepted 2026-09-08) commits to **implementation specifics**:
...
| | User-Generated Content | No (curated Shared Scenarios gallery; user-authored scenarios stay local to the device) |
...
### 4.4 Bundling strategy and category schema

The blocklist ships as a versioned JSON file at
`Pastura/Pastura/Resources/ContentBlocklist.json`. A separate
source-of-truth at `docs/blocklist/source.json` carries richer metadata
(`lang`, `source`, free-form `notes`); a jq-based build script
(`scripts/build-blocklist.sh`) extracts only the runtime fields (`term`,
`contentCategory`) and writes the bundled file. This split keeps audit
ergonomics rich for the curator while shipping the minimum to the bundle.

```jsonc
// docs/blocklist/source.json — git-only, not bundled
{
  "version": 1,
  "patterns": [
    { "term": "殺す", "contentCategory": "violence",  "lang": "ja", "source": "..." },
    { "term": "fuck", "contentCategory": "profanity", "lang": "en", "source": "..." }
  ]
}

// Pastura/Pastura/Resources/ContentBlocklist.json — bundled (term + contentCategory only)
{
  "version": 1,
  "patterns": [
...
**`contentCategory` enum.** Five values aligned with the OpenAI Moderation
taxonomy:
...
Field name `contentCategory` (not `category`) avoids collision with
`gallery.json`'s gallery-taxonomy `category` field.

**Input / output partition.** `ContentBlocklist.inputPatterns` excludes the
`violence` category; `ContentBlocklist.outputPatterns` includes all
categories. Authoring-time validator (§4) lets ethics/roleplay scenarios
use violence-topic words (e.g. "殺人事件" in the curated detective
scenario, "人を殺す選択" in trolley dilemma) while the output filter (§5)
still catches them at LLM-response time so defense-in-depth is preserved.

> **Amended 2026-04-27 — input/output category split.** The original
> §4.4 stated "Both layers block the same kinds of content. Divergence
> ... is speculative today." That paragraph is reversed by §10.1; the
> shared bundle remains a single source of truth, but the partition
> policy now diverges by design.

**Drift defense.** `bash scripts/build-blocklist.sh --check` validates
shape, `contentCategory` enum membership, and byte-level drift of the
bundled file against a fresh build from source. Wired into both the
pre-commit hook (gated on staged-file diff) and the `blocklist-drift` CI
job — stale bundles cannot land via local commit or merge.

**Selection criteria** (vocabulary expansion — Phase 2 of §9.2 row #5,
not this ADR):

- English coverage:
  [dsojevic/profanity-list](https://github.com/dsojevic/profanity-list)
  is the current preferred candidate (MIT, JSON-structured, ships
  severity / exceptions / partial_match flags that map cleanly to a
  future source.json schema extension).
  [LDNOOBW](https://github.com/LDNOOBW/List-of-Dirty-Naughty-Obscene-and-Otherwise-Bad-Words)
  remains a reasonable fallback but is en-US heavy and misses slang
  drift.
- Japanese coverage: no canonical ja-equivalent to LDNOOBW exists.
  [MosasoM/inappropriate-words-ja](https://github.com/MosasoM/inappropriate-words-ja)
  is the planned seed (carries Sexual / Sexual_minor / Offensive /
  Discriminatory groups that map to the schema's enum).
...
**Missing-resource posture.** `ContentBlocklist.entries(from:)` triggers
`preconditionFailure` on missing/unreadable resource, malformed JSON,
schema version mismatch (`expectedSchemaVersion: 1`), unknown
`contentCategory` value, an empty `patterns` array, OR a post-partition
`inputPatterns` that turns out empty (catches "all categories
accidentally excluded", which would silently disable the input layer).
All six are build-configuration / data-curation bugs that should fail at
launch, not silently degrade to an empty filter. The
`init(blockedPatterns:)` injection on `ScenarioContentValidator` and
`ContentFilter` is unaffected — tests can still pass `[]` explicitly
because the precondition lives at load time, not in the constructor.

**Future: dynamic delivery.** When the blocklist outgrows what bundled
app updates can reasonably ship (rough trigger: list size >100 patterns
or monthly-or-more emergency-update demand), the runtime file moves
behind a Shared Scenarios-style HTTPS fetch (see §6.6 for the curated-content
delivery pattern; the runtime contract would be the same — shape +
contentCategory + version). This is out of scope for §10.1's Phase 1
and is filed only when triggered.

### 4.5 Invocation points

The validator runs at two layers — sheet-level (per-field, per-tap, fast
feedback) and ViewModel-level (full scenario, backstop). Defense in
depth means residual blocked text that bypasses the inline gate (e.g.
fields hidden by a phase-type switch after the user typed in them) is
still caught at outer save.

**Sheet-level (per-field, fires on the editor sheet's "Save" tap):**

- `PersonaEditorSheet` — `name`, `description` validated via
  `validate(personaName:description:)` against the same shared
  blocklist; per-field findings render as inline `Color.danger`
  captions in the Section footers and the sheet stays presented when
  any field violates.
- `PhaseEditorSheet` — `prompt`, `template`, `condition` validated via
  `validate(phasePrompt:template:condition:)`. Visible-fields-only by
  design: only the fields the active `phase.type` exposes are checked
  (prompt for `requiresLLM`, template for `.summarize`, condition for
  `.conditional`). The injected validator is forwarded into the nested
  sub-phase `PhaseEditorSheet` so depth-1 conditional sub-phase Save
  uses the same blocklist instance.

Sheet-level findings are context-free (§4.7) — messages do not
interpolate sibling fields, so a both-fields-violating case cannot
leak the matched term via interpolation.

**ViewModel-level (full scenario, backstop):**
...
The existing `validate()` method already accumulates `validationErrors:
[String]`; content-validator findings append to the same array and use
the same error-display surface — the existing error banner in
`ScenarioEditorView` displays the messages.
...
3. Markdown export (simulation result → Share Sheet) — filtered before
   serialisation.
4. Past-results viewer (and D8 resume-replay) — `parsedOutputJSON` is
   persisted **unfiltered** (a structured mirror of the raw emission, like
   `rawOutput`), so the display surface applies `ContentFilter` at **read
   time** via `PersistedTurnDecoder.decodeFiltered`. (Earlier text here
   claimed persist-time filtering; that never matched the code — reconciled
   2026-07-13, see §10.3 and #1075.)
...
The filter reads `ContentBlocklist.outputPatterns` (§4.4) — all
categories, including `violence`, which the input validator skips.
Output filtering remains the defense-in-depth backstop for topic
words that the input layer intentionally permits. §9 registers
...
- The full pattern is never *displayed* unfiltered. The committed in-memory
  `AgentOutputRow` is filtered at commit; `TurnRecord.parsedOutputJSON` /
  `rawOutput` are persisted **unfiltered**, but every display surface that
  reconstructs a turn from them filters at read time (§5.1 item 4, §10.3), so
  a completed pattern never reaches a display surface.
...
2. **User report via the Shared Scenario reporting channel (§6).** If a
...
## 6. Shared Scenario Report Mechanism
...
**Shared Scenarios is curated, not user-generated.** Per
...
The Shared Scenario reporting mechanism is a **pseudonymous contact
...
- **In-app form → web form service (primary, chosen).** The report
  button in Shared Scenarios opens a sheet that displays scenario metadata
...
- **Internal target: first human response within 7 days, best-effort.**
  ...
  operations doc (`docs/gallery/shared-scenario-reports.md`).
...
curated-only Shared Scenarios — there are no users to block because users do
...
- **GitHub issue template**: `.github/ISSUE_TEMPLATE/shared-scenario-report.yml`
  mirrors the Google Form's field set; no "reporter contact" field is
  collected (public-tracker PII hygiene).
- **Operational procedure**: `docs/gallery/shared-scenario-reports.md` —
...
page (`pages/support/index.html`, deployed to
`https://pastura.app/support/`) rather than linking the raw GitHub
repo — iOS reviewers reject repo-only Support URLs (the quick-rss
precedent cited in #182).

> **Amended 2026-05-15 — canonical URL migration.** The Support URL
> host above migrated from `tyabu12.github.io/pastura/*` to
> `pastura.app/*` after the apex custom domain was configured (#389).
> Infrastructure only; the Support URL decision is unchanged. The
> previous host continues to 301-redirect, so existing submissions
> are unaffected. See §10.2.
...
- The confirmation template (`shared-scenario-reports.md` §2.1) is
...
ship with first — live in **ADR-006** (Accepted 2026-09-08). This ADR's
...
- **Privacy Manifest** (`PrivacyInfo.xcprivacy`, tracked in §9): declares
  the user-authored text sent off-device as a collected data type under
  `NSPrivacyCollectedDataTypes`. The manifest schema has no field for the
  *recipient* — that name is carried by the consent screen, App Privacy
  Details, and the privacy policy. (Corrected in place 2026-09-08 by
  ADR-006 Decision 6; the original bullet asked the manifest to "list the
  third-party destination", which it cannot express.)
...
4. Privacy policy / App Privacy Details naming the provider, and the
   Privacy Manifest declaring the data type (disclosure layer; corrected
   2026-09-08 with the §7.3 bullet).
...
- Network request auditing: the app's entitlements do include network
  access (for Shared Scenarios gallery fetches, model download, and future
...
- **§5 filtering policy is now binding** on every new display surface.
  ...
  tightening pass.
- **Cloud API work is scoped by ADR-006.** §7's principles bound ADR-006
  but do not authorise implementation. ADR-006 was Accepted 2026-09-08;
  engineering work on Cloud API now follows its §8 slices in order
  (corrected in place 2026-09-08 — until then this bullet read "gated on
  ADR-006 … out of scope until ADR-006 merges").
...
| 4 | Shared Scenario report UI (§6) + pseudonymous contact surface | [#178](https://github.com/tyabu12/pastura/issues/178) | tyabu12 | Soft — §1.2 does not strictly apply to curated content, but defensive posture recommended before review | The §6 Google Form also co-tenants as the §1.5 general-contact surface via the Support URL landing page — see §6.7 and row #9 |
| 5 | Blocklist category schema (Phase 1) + vocabulary expansion (Phase 2 — dsojevic / MosasoM seeds) (§4.4 / §10.1) | [#257](https://github.com/tyabu12/pastura/issues/257) | tyabu12 | None (ongoing) | Phase 1 (JSON schema + input/output partition + drift guard) shipped via #257 (2026-04-27). Phase 2 (vocabulary seeds) is data-only and triggered by App Review citation, telemetry, or user report. Future: dynamic DL when scale warrants — see §4.4. |
...
| 7 | ADR-006 Cloud API disclosure/consent implementation | ADR-006 Accepted 2026-09-08 ([#1692](https://github.com/tyabu12/pastura/issues/1692)); implementation tracked as its §8 slices (a)–(c) | tyabu12 | Cloud API feature (not submission) | Principles from §7 bind this work; slice (c) is the disclosure release gate |
...
| 9 | App Store Connect Support URL landing page (§6.7 / §10.2) | [#182](https://github.com/tyabu12/pastura/issues/182) | tyabu12 | **Submission** | Filed 2026-04-21; `pages/support/index.html` deployed to `https://pastura.app/support/` (canonical host migrated 2026-05-15 — see §10.2) |
...
- `docs/gallery/README.md` — Shared Scenarios trust model and curation
  rules; cited in §6.1.
- `Pastura/Pastura/App/ContentFilter.swift` — current filter
  implementation; cited in §5 and §4.6 (actor-isolation contrast).
- `Pastura/Pastura/Resources/ContentBlocklist.json` — runtime bundled
  blocklist (term + contentCategory only); cited in §4.4, §5.2.
- `docs/blocklist/source.json` — git-only source-of-truth with
  metadata (lang, source, notes); cited in §4.4.
- `scripts/build-blocklist.sh` — jq-based runtime-JSON generator and
  drift checker; cited in §4.4.
...
- Issue #257 — Phase 1 of §9.2 row #5 (category schema + input/output
  partition); cited in §10.1.

---

## 10. Amendments

This section records post-implementation amendments (policy or
infrastructure) to ADR-005. Per CLAUDE.md "Decision Records",
amendments use a top-level `§N` heading rather than mutating the
original section in place — this preserves an audit trail of which
decisions changed mid-flight and why. The amended section retains
a one-line pointer to the corresponding amendment subsection here.

### 10.1 (2026-04-27) Input/output category split

**Section amended:** §4.4 (Bundling strategy and category schema).

**Sentence reversed.** Original §4.4 stated:
...
This is reversed by introducing a `contentCategory` schema and
partitioning the shared blocklist: `ContentBlocklist.inputPatterns`
excludes the `violence` category while `outputPatterns` retains all
categories.

**Why reversed.** ADR-005 §6 commits to a curated-only Shared Scenarios gallery.
Two of the three curated gallery seeds (`trolley_dilemma_v1.yaml`,
`detective_scene_v1.yaml`) contain violence-topic words ("人を殺す
選択", "屋敷で発生した殺人事件") that are essential to their
ethics/roleplay framing. Under the original flat blocklist,
`ScenarioContentValidator` (§4) rejected these scenarios at
authoring/install time — a curation contradiction with §6's
defensive curator-trust posture. The original "speculative today"
assertion did not anticipate that gallery curation itself would
surface a divergence requirement.

**What stays.** The shared bundle remains a single source of truth
(no per-layer files). Defense-in-depth is preserved — the output
filter (§5) catches the violence partition that the input layer
skips, so LLM responses cannot emit unfiltered violence-topic
content. The `init(blockedPatterns:)` injection contract on both
types is unchanged.

**Implementation:** Issue [#257](https://github.com/tyabu12/pastura/issues/257)
(2026-04-27).

**Future reversibility.** If a third-party review re-folds violence
into the input partition (e.g. App Review citation flagging gallery
content), the change is one line:
`ContentBlocklist.categoriesExcludedFromInput = []`. The schema
admits the reversal without a further amendment.

### 10.2 (2026-05-15) Canonical URL migration to apex domain

**Sections amended:** §6.7 (Support URL co-tenancy), §9.2 row #9
(sub-issue master index).

**Not a policy change.** This is an infrastructure-only amendment.
The §1.5 Support URL and Apple Privacy Policy URL decisions remain
unchanged. Only the canonical host migrated from
`tyabu12.github.io/pastura/*` to `pastura.app/*` after the apex
custom domain was configured (#389).

**What changed.** The Support URL reference in §6.7 and the §9.2
row #9 deployment URL now point at the apex domain. GitHub Pages
issues a permanent 301 from the previous `tyabu12.github.io/pastura/*`
host, so the App Store Connect Support URL filing (filed 2026-04-21,
row #9 status `Submission`) and the in-app `Settings → Privacy
Policy` link (`Pastura/Pastura/Views/Settings/SettingsView.swift`)
continue resolving without operator intervention. The in-app link
will route directly once the next TestFlight rebuild ships; ASC
URL re-pointing remains a separate operator action and is out of
scope for this amendment.

**What stays.** All compliance posture from §6.7 (dedicated landing
page, not raw-repo) and §9.2 row #9 (filed status, sub-issue
linkage) is preserved. The 301 chain keeps existing review
submissions, cached social-card previews, and the current in-app
build working until the operator-driven ASC update and the next
TestFlight push land.

**Implementation:** Issue
[#390](https://github.com/tyabu12/pastura/issues/390) (2026-05-15)
following apex DNS + CNAME setup in
[#389](https://github.com/tyabu12/pastura/issues/389).

### 10.3 (2026-07-13) `parsedOutputJSON` read-time filtering

**Sections amended:** §5.1 (display-surface inventory, item 4), §5.3
(partial-prefix-leakage rationale).

**Not a policy change — a doc/code reconciliation.** The §5.1 invariant
(every user-visible LLM-output display surface passes through
`ContentFilter`) is unchanged. The prior §5.1 item 4 / §5.3 text asserted
that `TurnRecord.parsedOutputJSON` was filtered *before storage*; that never
matched the code — `SimulationViewModel.persistTurnRecord` persists the
unfiltered parse, and the past-results viewer decoded it without filtering,
an existing deviation from the §5.1 invariant on that one surface (#1075).

**What changed.** The invariant is now honoured at **read time**:
`parsedOutputJSON` (and `rawOutput`) are documented as persisted unfiltered
— structured / literal mirrors of the raw emission — and every display
surface reconstructing a turn routes through the shared
`PersistedTurnDecoder.decodeFiltered`, which applies `ContentFilter` on
read. The D8 resume-replay path (`ResumeLogReplayMapper`) already filtered
and now shares that decoder; the past-results viewer
(`ResultDetailView.decodeTurnOutput`) previously did not and now does.
Read-time was chosen over persist-time filtering so already-persisted
unfiltered records are repaired without a migration; `ContentFilter.filter`
is idempotent, so the share / Markdown / YAML export paths that re-filter
stay safe.

**Why §5.3 still holds.** Partial-prefix leakage is a *display-timing*
residual risk. The reconciled §5.3 wording ("never *displayed* unfiltered")
preserves the acceptance rationale: a completed blocked pattern is never
shown on any surface, even though the raw text is persisted (as `rawOutput`
always was). §5.4 reconsideration triggers are unaffected.

**Implementation:** [#1075](https://github.com/tyabu12/pastura/issues/1075).
