...
| `App/SimulationViewModel` suite Logger interpolations + BG identifiers (63 candidates, re-measured 2026-08-07; the prior 50 had drifted independently of this row's last edit) | `Pastura/Pastura/App/SimulationViewModel.swift`, `Pastura/Pastura/App/SimulationViewModel+Background.swift`, `Pastura/Pastura/App/BackgroundSimulationManager.swift` | **Permanent** — 53 `os.Logger` `info` / `notice` / `error` interpolations (the residual after the three categories below — 53 + 6 + 2 + 2 = 63, so the itemization sums to the header count) annotated `privacy: .public` per CLAUDE.md Logger-privacy rule (lifecycle / scenePhase / BG-task diagnostics like `"scenePhase=.active enter: isRunning=%arg, ..."`, `"BG task activation: isRunning=%arg, ..."`, `"committed agent=%arg totalAttempts=%arg"`); 6 Logger subsystem / category strings (`app.pastura.Pastura` × 3 — `SimulationViewModel.swift` declares two Loggers — + `SimulationVM`, `StreamingDiag`, `BGSimManager`; the earlier `× 2` undercounted, independently of this row's other edits); 2 BGTaskScheduler identifier fragments — the identifier is derived from the bundle ID (the Debug configuration suffixes it), so the audit sees `app.pastura.Pastura` as the `??` fallback plus `%arg.simulation-continuation` as the interpolation template, rather than one literal; 2 `cancelSimulation(caller:)` debug tags (`switchToCPUInference-error`, `switchToGPUInference-error`). Diagnostic-only, never user-facing UI strings. Per #340 slice-7 decision. |
...
| `accessibilityIdentifier("editor.*")` UI-test selectors | `Pastura/Pastura/Views/Editor/ScenarioEditorView.swift:99,122` | **Permanent** — `accessibilityIdentifier(_:)` strings are UI-test programmatic selectors consumed by `PasturaUITests` (e.g., `EditorReloadTests`), NOT VoiceOver-spoken labels (those go through `accessibilityLabel(_:)`, which IS wrapped). Identifier stability over locale display per PR #376 (`accessibilityLabel` vs `accessibilityIdentifier` distinction; the `.accessibilityLabel(_:)` triage rule lives in `.claude/rules/i18n-ui.md` § Audit triage). Per #340 slice-10 decision. |
| `LlamaCppService` Logger + signpost + KV-cache mode tokens (17 candidates) | `Pastura/Pastura/LLM/LlamaCppService.swift` | **Permanent** — Three sub-classes: (a) `os.Logger` subsystem / category strings — `"app.pastura.Pastura"` × 3, `"LlamaCppService"`, `"StreamingDiag"`, `"LlamaCppRuntime"` (the last is the llama.cpp C-runtime log-capture channel installed via `llama_log_set`). (b) `os.signpost` trace labels passed as plain `StaticString`-like literals — `"loadModel"`, `"unloadModel"`, `"generateStream"`. (c) `os.Logger.debug` interpolations annotated `privacy: .public` per CLAUDE.md Logger-privacy rule — `"streamCheckpoint mode=… tokens=… tail=…"` (DEBUG-only per-loop checkpoint), `"decodeFailureError: result=…, suspendRequested=…"` (Metal decode-failure diagnostic), `"generate enter: isModelLoaded=…, …"`, `"generate post-throttle: …"`, `"generate throwing .notLoaded: …"` (load/reload race-investigation trace), `"<stopSequence> stop sequence detected — ending generation/stream early"` × 2 (interpolated form). Plus the KV-cache mode label (`"non-stream"`) used as the signpost `mode=` argument value. Diagnostic-only, never user-facing UI strings. Per #340 slice-11 decision. |
...
| `App/` external-tool input + internal path-template internals (2 files) | `Pastura/Pastura/App/ScenarioGenerationPrompt.swift`, `Pastura/Pastura/App/ScenarioContentValidator.swift` | **Permanent** — External-tool prompt + internal diagnostic field-paths. Two sub-classes: (a) `ScenarioGenerationPrompt.text` (computed `static var`, generated from `PhaseType.allCases` / `ScoreCalcLogic.allCases` via no-default switches, plus a raw `format.md` URL) — prompt template **passed to an external LLM via clipboard** (user copies to ChatGPT / Claude / etc. to generate a Pastura YAML scenario). ADR-010 §4 Engine exclusion analog: content is structurally bound by what the receiving LLM should output (Pastura YAML schema with `phase_type` discriminators + `language: ja|en` directive). MUST stay English regardless of user locale because (1) curated example scenarios are bilingual via sibling YAMLs, (2) the embedded YAML field names (`id`, `language`, `personas`, `phases`, …) are wire-format tokens consumed by `ScenarioLoader`. Same shape as slice-11 LLM-bucket decision. (b) `ScenarioContentValidator.swift:195/L202` JSON-path key templates `"%arg.then.%arg"` / `"%arg.else.%arg"` — conditional-phase recursion position labels (e.g., `2.then.1`, `2.else.3`) embedded as the `%arg` position interpolation inside localized findings like `"Phase %arg prompt contains a term that is not allowed"`. Field-path internals, not user-readable English prose. Per #340 slice-13 decision. |
| Share-to-X bucket: brand tag + Logger / identifier + web-intent query tokens (~11 candidates) | `Pastura/Pastura/Views/Components/ShareHashtag.swift`, `Pastura/Pastura/Views/Components/ShareCaptionItemSource.swift`, `Pastura/Pastura/Views/Components/XPostSharer.swift` | **Permanent** — Two sub-classes. (a) Brand hashtag (`ShareHashtag.name` = `"Pastura"`, `.hash` = `"#Pastura"`) carried by X-bound shares. Identical in every locale by definition: a catalog entry would let the `ja` and `en` values drift into two different tags, splitting the tag's own search results. Consumed two ways — as X's `#`-less `hashtags=` web-intent param (`ScenarioShareSheet.postToX`) and as text appended to the system-share caption (`ShareCaptionItemSource`) — with `hash` derived from `name` so the two forms cannot diverge. Same shape as the `PlaybackSpeed` multiplier-notation carve-out (slice 12): canonical convention statement lives in the `ShareHashtag` doc-comment at the source file, and `XPostSharerTests.shareHashtagForms` literal-pins both forms as the regression guard against accidental wrap. (b) `ShareCaptionItemSource` diagnostic + wire identifiers — `os.Logger` subsystem / category strings (`"app.pastura.Pastura"`, `"ShareCaption"`), the destination-observation format string `"share destination activityType=%arg"` (annotated `privacy: .public` per the CLAUDE.md Logger-privacy rule, plus its `"nil"` fallback token), and the pinned X share-extension bundle identifier `"com.atebits.Tweetie2.ShareExtension"` (an Apple/vendor `UIActivity.ActivityType` raw value — localizing it would break destination matching outright), plus the caption/tag join template `"%arg %arg"`. (c) `XPostSharer` web-intent query tokens — `"text=%arg"`, `"&url=%arg"`, `"&hashtags=%arg"`, `"%arg?%arg"` (base + query join). URL query-parameter names are bound by X's Web Intents contract (docs.x.com/x-for-websites/post-button), same shape as the `URLSessionGalleryService` HTTP/URL wire-token clause above; translating one would simply drop the param. Diagnostic-only / wire-format, never user-facing UI. |
...
| `App/UITestSupport/` fixtures + DEBUG-gated entry-point error (3 entries) | `Pastura/Pastura/App/UITestSupport/{StubGalleryService,StubScenarioSeeder,StubScenarioSeeder+Localized,StoreScoreboardSample}.swift`, `Pastura/Pastura/PasturaApp.swift:523` | **Permanent — `--ui-test`-gated UI-test fixture / DEBUG-only entry-point error**. Three sub-classes: (a) UI-test fixture YAML + IDs — `StubGalleryService.canaryYAML` (L51-69) + `canaryYAMLURL` (`"stub://gallery/canary.yaml"`) + scenario id `"ui_test_canary"`; `StubScenarioSeeder`'s per-language Home fixtures (`homeSeedYAML{En,Ja}`, `richDilemmaYAML{En,Ja}`, `richDesertYAML{En,Ja}`, `richWordWolfYAML{En,Ja}`) + `editorSeedYAML` + persona names (`Alice`, `Bob`, `Carol`, `Dave` — same shape as the `SheepAvatar` Preview-only `Character` carve-out above). `StoreScoreboardSample.japanese` (ハルカ / ケンタ / ミサキ / リュウジ — the fixed scoreboard the store capture photographs) is the same class of literal and belongs to this same carve-out. The `*Ja` variants are **hardcoded Japanese**, which is correct here and not a leak in reverse: these are fixture *data* selected by `LocaleResolver.deviceDefault()` at seed time, not UI copy resolved through the catalog — and `#if DEBUG` strings are not auto-extracted anyway (see § "`#if DEBUG` strings are not auto-extracted to the catalog" in `.claude/rules/i18n-ui.md`), so routing them through `Localizable.xcstrings` would add translator-facing keys that no user-facing surface reads. Both files are wrapped in `#if DEBUG` and consumed only by `PasturaUITests` (canary navigation + Home seed + editor reload tests). (b) UI-test bootstrap fatal-error literal — `StubGalleryService.swift:44` `fatalError("Canary YAML URL literal failed to parse")` (compile-time literal failure-impossible by construction; project bans `!` so the guard is explicit). (c) `PasturaApp.swift:523` `appState = .error("UI test setup failed: \(error.localizedDescription)")` — DEBUG entry-point error inside `setupUITestState()` body. **Two-gate condition** `#if DEBUG` (L500) AND `CommandLine.arguments.contains("--ui-test")` (L385); routes into `Text(message)` at L220 only inside `PasturaUITests`-launched processes, never reaches end users. The asymmetry with the production-path siblings L404 / L496 (Database error: `do/catch` in `initialize()` / `finalizeInit()`) and L479 (No active model descriptor: `guard let descriptor` in `finalizeInit()`) — all wrapped in slice-13 — is intentional: the production `.error` paths fire in real user flows, while L523 fires only inside `xctest` host processes. Same gating pattern as the StubGalleryService / StubScenarioSeeder fixtures. Per #340 slice-13 decision. |
| `Engine/ConditionEvaluator.swift` runtime-absent condition warnings (7 candidates) | `Pastura/Pastura/Engine/ConditionEvaluator.swift` | **Deferred** — The `warnings` strings `resolveDerived` appends for a runtime-absent variable (`max_score` / `min_score` / `vote_winner` / `vote_winner_count` "has no value …", plus the `scores.<Name>` and `state.variables` misses). Unlike the Logger carve-outs above these ARE user-reachable: `ConditionalHandler` forwards them to the `.summary` warning channel. All existing siblings are equally un-wrapped, so `vote_winner_count` (#1599) inherits the gap rather than opening it — registered here so the audit stops re-discovering it per pass. Gating event: wrapping the set as one batch, which also has to settle whether these are author-debugging diagnostics (engine-language, like a lint finding) or player-facing copy. |
