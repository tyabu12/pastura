...
| UR-003 | 2026-06-24 | L5 | 07-results | `"%lld records"` not plural-aware (renders "1 records") | done | [compliance-gap] Fixed: callsite → `Text("\(count) records")` (Int-interp plural); en `variations.plural` one/other + ja single-form; `extractionState=manual` (key invisible to the CLI extract). Added § Plurals (now `i18n-ui.md`, split out in #1431) + a catalog-structure test. |
