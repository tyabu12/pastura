...
> **Status:** Accepted — GO (KMP Models spike #220; H5/H7 distribution-verification discharged 2026-09-05 on `v1.3+886` — §11; ADR-023 Decision 6 (iii) discharged 2026-09-06 on `v1.3+888` — §12, status unchanged; the §9.3 unmeasured Tier-2 gates stay non-blocking watches)
...
>
> **Update 2026-09-05.** H5 and H7 were executed on the ADR-023 S5-3 TestFlight
> cycle (`v1.3+886`) and both passed, so the condition attached above is
> discharged and the verdict is **GO** — §11.
...
spike (§9), as a **Conditional GO** (2026-06-11): every Models-layer-testable
Tier-1 hypothesis was green and the spike over-delivered against §8's KMP-PoC
gating criterion (full Models layer ported, not a single handler). Two
distribution-pipeline Tier-1 blockers (H5 ASC archive upload, H7 crash
symbolication) were carried as named launch-prep actions (§9.2) — ASC-onboarding
tasks, not KMP-capability gaps — and were **executed and passed on 2026-09-05**
(§11), which graduates the verdict to **GO**. Phase 3 implementation started on
2026-09-04 once the §8 criteria cleared; the architecture choice itself is no
longer tentative.
...
The KMP-PoC criterion above is satisfied by the #220 spike (§9), which moved
this ADR from Draft to **Accepted — Conditional GO** on 2026-06-11 (the
condition was discharged 2026-09-05, §11). The remaining criteria
...
canonical H7 (crash symbolication) were unexecuted at acceptance and were DEFER
(§9.2) — both executed and passed 2026-09-05 (§11).**
...
| **H5** | Archive builds + uploads to ASC; processing succeeds (no invalid-binary rejection) | ✅ **PASS** (2026-09-05) | Deferred at acceptance (§9.2); executed on ADR-023 S5-3: `v1.3+886` archived with the K/N umbrella, fastlane upload waited for ASC processing and returned success — no invalid-binary rejection ([#501](https://github.com/tyabu12/pastura/issues/501#issuecomment-5550162180)). §11 |
...
| **H7** | Intentional Kotlin crash → symbolicated frames in Xcode Organizer Crashes view (via K/N dSYM) | ✅ **PASS** (2026-09-05) | Deferred at acceptance (§9.2); executed on ADR-023 S5-3: two named `com.pastura.engine.H7CrashProbe` frames with Kotlin file:line in App Store Connect's own crash view, K/N UUID matching the archived dSYM; the archive dSYM resolves the same addresses locally ([#501](https://github.com/tyabu12/pastura/issues/501#issuecomment-5550162180)). §11 |
...
**Tier-1 reading (2026-06-11):** 4 PASS (H1, H2, H6, H8), 1 proxied-N/A (H4),
2 DEFER (H5, H7). Every hypothesis a **Models-only** spike can test was green;
the two DEFERs and the H4 proxy were all consequences of scope boundaries
(Engine not ported) or distribution-pipeline onboarding (below) — none a
KMP-capability failure. **As of 2026-09-05:** 6 PASS, 1 proxied-N/A (H4) — the
two deferrals are discharged (§11).

### 9.2 The H5/H7 deferral — named launch-prep actions

> **Discharged 2026-09-05 (§11).** The bullets below are the 2026-06-11 deferral
> rationale, kept as written; the execution moved from the #233 launch-prep
> axis to ADR-023 §6 S5-3, because the H7 crash needs a TestFlight build that
> *carries* the K/N umbrella, which the app only did from Stage 5.

H5 and H7 were **executable-but-unexecuted**, not structural blockers:
...
clean GO as "Tier 1 all pass"; this was a **Conditional GO** that explicitly
carved out H5/H7 as distribution-verification actions, per operator decision
(2026-06-11). The Conditional GO would have been voided (→ re-evaluate) only if
H5 surfaced an ASC invalid-binary rejection of the K/N XCFramework (risk R7) or
H7 showed K/N frames do not symbolicate — neither of which any spike evidence
predicted, and neither of which fired (§11).
...
| Q8 | crash symbolicated in debug runs | ✅ PASS (2026-09-05) | Was NOT MEASURED (tied to the deferred H7 path). H7's release-build TestFlight crash symbolicated in ASC and against the archive dSYM (§11) — a stronger reading than the debug-run target |
...
**Tally (2026-06-11):** 4 PASS (Q1-cold, Q3, Q4-warm, Q5), 3 PARTIAL (Q1-warm,
Q4-cold, Q9), 4 NOT MEASURED (Q2, Q6, Q7, Q8). Measured-pass ≈ **4/11 (≈ 36 %)**,
or **7/11 (≈ 64 %)** counting partials — **below the literal 80 % gate**. With Q8
passing on 2026-09-05: 5 PASS / 3 PARTIAL / 3 NOT MEASURED — 5/11 (≈ 45 %), or
8/11 (≈ 73 %) counting partials; still below the literal gate, on the same
honest framing below.

**Honest framing:** the verdict was a **Conditional GO on the measured Tier-2
subset** — every gate that *was* measured passes or partially passes, and the
unmeasured gates were local-developer-ergonomics (Q2/Q6/Q7) plus crash
symbolication (Q8, which rode on the deferred H7 and passed with it), none of
them KMP viability blockers. Two of the partials carry real watches: **Q4-cold**
breaches the 20-min CI ceiling under full cache miss (konan pre-warm needed —
§9.7), and **Q9**'s shim budget is proven on only 8/21 types. The spike's
headline Tier-2 motivation — simulator-free business-logic test execution (Q3)
— is firmly green. The operator judged the measured evidence sufficient for
Conditional GO; H5/H7 were discharged on 2026-09-05 (§11), and the remaining
unmeasured gates (Q2/Q6/Q7) and the two watched partials stay **non-blocking
watches** on the GO, not silent passes.
...
The H5/H7 distribution-verification actions (§9.2) sat on a *separate* axis —
Phase 2 App Store launch onboarding (#233), not this Phase 3.0 code-merge; they
were in the end executed under ADR-023 §6 S5-3 on 2026-09-05 (§11).

> **Update 2026-07-12 (#501 Stage 1, PR-1).** ADR-023 §6 re-sequenced the merge:
> `shared/models` now lands on `main` at **Stage 1 as infrastructure**
> (dead-sandbox / not-production-wired), *decoupled* from the first K/N-consumer
> PR — so the "bundled with the first PR that consumes the K/N Models" clause
> above is no longer the merge plan of record (see ADR-023 §6 Stage 1 / Stage 5
> and the #501 Stage-1 plan). Landmine 1 is discharged in PR-1 by authoring a
> gate-free `kmp-build-test` job (the `on:` triggers already restrict to
> main-events, so there is no `base_ref == 'main'` skip to mis-set). Landmine 2's
> rebase guard held: PR-1 used a selective byte-identical import of the keep-set
> from `f73bc48`, which preserves this reconciled §9 (the branch's older "W6
> merge" text was never pulled in). T11 removal was a no-op — `main`'s `on:`
> never carried the spike-branch triggers. `feature/kmp-spike-models` is retained
> (not deleted) as the Stage-2/5 salvage source for the stripped Swift consumer
> scaffolding.

**Closed 2026-09-07 at ADR-023 S5-5**
([#1685](https://github.com/tyabu12/pastura/issues/1685); §13). Both carried items were
discharged earlier — landmine 1 by the Stage-1 PR-1 gate-free `kmp-build-test` job, landmine
2 by the Phase 3.0 code-merge itself, whose rebase guard held. The remaining spike close-out
is now done too: the Stage-2 gate spike (`tools/kmp-gate-spike/`) is retired, its non-parity
assets re-homed — the B′ isolation guard to `scripts/kmp/check-b-prime-isolation.sh` — and
the `macosArm64` parity rung left untouched (ADR-023 §16). The H5/H7 axis this section
routed elsewhere closed separately in §11. Nothing in §9.7 remains open.
...
## 11. Amendment 2026-09-05 — H5/H7 discharged: Conditional GO → GO

The two Tier-1 hypotheses §9.2 deferred were executed on the ADR-023 §6 **S5-3**
TestFlight cycle and both passed. Evidence, verbatim, is the operator's
[#501 comment](https://github.com/tyabu12/pastura/issues/501#issuecomment-5550162180);
the procedure is `docs/qa/h7-symbolication-qa.md`.

**What ran.** Tag `v1.3+886` (build 886, HEAD `65f2b90a`), cut with
`scripts/release.sh` on 2026-09-05. A first attempt, `v1.3+885`, uploaded and
processed (an H5 pass in itself) but could not reach the crash: the Diagnostics
reveal gesture never attached because `BuildChannel` mapped a StoreKit
`SKInternalErrorDomain Code=13` to "not sandbox". #1678 added the receipt-name
fallback and the cycle was re-cut as 886.

**H5 — PASS.** The fastlane upload lane waited for App Store Connect processing
and returned success; the tag was pushed. No invalid-binary rejection of the
K/N XCFramework — risk R7 did not fire. The release script's K/N checks read:

```
▸   ✓ K/N dSYM present; UUID(s): D11803D3-87D9-368E-8678-94EFEC5ACDEC
▸   ✓ Symbols/D11803D3-87D9-368E-8678-94EFEC5ACDEC.symbols present in the .ipa
```

**H7 — PASS, both halves.** The intentional Kotlin crash (`H7CrashProbe`, fired
from Settings › Diagnostics on iPhone17,5 / iOS 26.6) reached App Store Connect
→ TestFlight → Crashes within minutes, symbolicated **from the uploaded
symbols** — the local archive copies had been moved out of the Organizer library
first, so no local dSYM could have resolved them:

```
0   PasturaSharedEngine   kfun:com.pastura.engine.H7CrashProbe.throwCrash#internal + 148 (H7CrashProbe.kt:34)
1   PasturaSharedEngine   kfun:com.pastura.engine.H7CrashProbe#crash(kotlin.String){} + 12 (H7CrashProbe.kt:27)
2   PasturaSharedEngine   objc2kotlin_kfun:com.pastura.engine.H7CrashProbe#crash(kotlin.String){} + 280
3   Pastura               static H7CrashTrigger.fire() + 72 (H7CrashTrigger.swift:33)
```

The pass criterion was one named `H7CrashProbe` frame; two resolved with Kotlin
file:line (the release build did not inline the private helper). The report's
binary-image UUID for `PasturaSharedEngine`
(`d11803d3-87d9-368e-8678-94efec5acdec`) equals the `kn-dsym` UUID logged at
archive time, so the staged XCFramework was the linked one. The local half —
`atos` against the archive's K/N dSYM at the report's load address — resolves
the same three addresses to the same symbols. Q8 (§9.3) passes on the same
evidence, at a stronger reading than its debug-run target.

**Consequences.**

- **Status → `Accepted — GO`.** Neither voiding condition §9.2 named (R7; K/N
  frames not symbolicating) fired. §4, §8, §9.0, §9.1, §9.2, §9.3 and §9.7 are
  reframed above to dated past tense with a pointer here; the 2026-06-11 text is
  kept as the acceptance rationale.
- **What the GO still carries.** The §9.3 unmeasured Tier-2 gates (Q2, Q6, Q7)
  and the two watched partials (Q4-cold, Q9) remain **non-blocking watches** —
  unchanged by this amendment, which discharges only H5/H7.
- **ADR-023 Decision 6.** Only sub-condition **(ii)** (H7 executed) is
  discharged by this cycle. (i) Stage-4 parity was already discharged when
  Stage 4 closed (2026-08-30, both rungs green — *corrected 2026-09-06; this
  bullet originally bundled (i) with (iii) as S5-4's*), and (iii) the
  switched-engine soak is S5-4's — discharged 2026-09-06, §12. Decision 6
  *fires* at S5-5, not here.
- **Release script.** `scripts/release.sh`'s `kn-dsym` / `kn-symbols` checks,
  warning-only for this first observation, are promoted to fail the release
  before upload (close-out issue #1679). The observation covered one UUID / one
  arm64 slice.
- **Standing obligation.** The H7 probe (Kotlin `H7CrashProbe`, the Settings
  Diagnostics reveal) must be deleted at S5-5 **before the next App Store
  submission** — the reveal gesture is reachable under App Review (ADR-023 §6
  S5-3 ordering constraint).

## 12. Amendment 2026-09-06 — ADR-023 Decision 6 (iii) discharged

The switched-engine soak that §11 left open as S5-4's was executed and passed.
Evidence, verbatim, is the operator's
[#501 comment](https://github.com/tyabu12/pastura/issues/501#issuecomment-5559576459);
the procedure is `docs/qa/kmp-soak-qa.md`.

**What ran.** Tag `v1.3+888` (build 888, carrying the S5-4 flag-gated switch
#1681), on an iPhone 16e with the device language set to `ja`, with
`FeatureFlags.sharedEngineEnabled` on for the whole cycle. The four presets the
runbook names — of the six bundled — completed on the Kotlin engine:
`target_score_race`, `word_wolf`, `last_fable`, `prisoners_dilemma`, selected
because between them they cover `conditional`, `event_inject`, and the ADR-027
`pairwise_payoff` scoring path. The streaming row carried extracted statement
text rather than raw JSON, so the S5-4 device-QA regression fixed on #1681 stays
fixed. Pause→resume and
background→foreground both continued the run to completion on the Kotlin engine;
kill→reopen resumed through the **Swift** runner, which is the designed behavior
(the Kotlin engine exports no resume-from-state, ADR-023 §6 S5-4). The
Diagnostics sample row rendered 「無効な YAML 形式です」, so the `appleMain`
`localizedFormat` actual resolves on device rather than falling back to its key.
App Store Connect showed no new crash for the build.

**Consequences.**

- **ADR-023 Decision 6 is fully satisfied.** (i) Stage-4 parity closed
  2026-08-30, (ii) H7 discharged 2026-09-05 (§11), (iii) discharged here. The
  Swift Engine nonetheless remains the shipping engine until **S5-5** flips the
  default — the S5-4 switch is opt-in and off by default. Decision 6 *fires* at
  S5-5 (issue #1685), and §9.7 closes with it.
- **Status unchanged.** `Accepted — GO` since §11; this amendment discharges an
  ADR-023 sub-condition, not an ADR-004 gate. The §9.3 unmeasured Tier-2 gates
  (Q2, Q6, Q7) and the two watched partials (Q4-cold, Q9) remain non-blocking
  watches.
- **Standing obligation unchanged.** §11's H7-probe deletion deadline still
  stands: it must land at S5-5 **before the next App Store submission**. —
  *discharged 2026-09-07 at S5-5; see §13*

## 13. Amendment 2026-09-07 — ADR-023 S5-5 landed; §9.7 closed

ADR-023 Decision 6 **fired** on 2026-09-07 with S5-5
([#1685](https://github.com/tyabu12/pastura/issues/1685), ADR-023 §16): the Kotlin engine is
the shipping engine for every fresh run, the Swift Engine remaining only for `resume()`.

- **§9.7 is closed.** Both Phase 3.0 code-merge landmines were already discharged, and the
  remaining spike close-out landed here — the Stage-2 gate spike is retired with the B′
  guard re-homed to `scripts/kmp/check-b-prime-isolation.sh`; the `macosArm64` parity rung
  is untouched.
- **§11's standing obligation is discharged.** The H7 probe and its Diagnostics reveal
  gesture are deleted, so nothing of that surface can reach App Review.
- **Status unchanged.** `Accepted — GO` since §11; this amendment closes an ADR-004 section,
  not an ADR-004 gate. The §9.3 unmeasured Tier-2 gates (Q2, Q6, Q7) and the two watched
  partials (Q4-cold, Q9) remain non-blocking watches.
