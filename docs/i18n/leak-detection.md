...
| `current_event` TextField placeholder | `Pastura/Pastura/Views/Editor/PhaseEditorSheet+EventInjectSection.swift:45` | **Permanent** — placeholder text is the literal `eventVariable` default value the phase falls back to when the field is empty (see inline comment lines 42–44 and the section footer copy). Localizing would diverge from the model-layer default, breaking the curator's mental model that the placeholder = the YAML token written when blank. Per #340 slice-10 decision. |
