...
> fixes (「心の声」, 「エージェントたち」) are applied below.
...
```
AI エージェントが推論し、駆け引きし、時に嘘をつく。すべて iPhone の中で、完全オフラインで。新しいシナリオやモデルも随時追加。アカウント登録は不要、データは端末の外に出ません。
```
...
できること
・ワードウルフや囚人のジレンマなど、内蔵シナリオを実行
・ビジュアルエディターで独自シナリオを作成。YAML に切り替えれば細部まで制御
・厳選された共有シナリオのギャラリーを眺め、ワンタップでインポート
・ローカルモデルを切り替えて、同じシナリオがどう変わるかを見る
・発言、心の声、投票、スコアがリアルタイムに届く
・実行結果を Markdown でエクスポートして、保存・貼り付け・共有

なぜローカル処理か
モデルは他人のサーバーではなく、あなたの端末の中にあります。それだけで、プライバシー、コスト、レイテンシが一度に片付く。推論がサーバーに触れることは一度もありません。月額は 0 円。テレメトリも分析もなし。あなたのシナリオも実行結果も、端末の外には出ません。

モデルを初めて選んだとき、Pastura はそれを一度だけダウンロードします（約 3 GB、Wi-Fi 推奨）。あとはずっとオフライン。ダウンロードを待つあいだは内蔵のデモ再生が流れ、モデルが揃う前から「観測」がどんなものかを確かめられます。
...
動作には 6.5 GB 以上の RAM を搭載した端末が必要です（iPhone 15 Pro 以降）。無料。サブスクリプションもアプリ内課金もありません。
```

**1,080 / 4,000 chars.** ✓ — Pre-fold opener: 28 chars (`AI観測。天体観測のように、ローカル LLM を眺める。`). ja reads shorter than en by design (natural density, not padded).
...
| # | Screen | Caption |
|---|---|---|
| 1 | シミュレーション実行中（発言＋心の声＋タイピング演出） | エージェントの思考を、リアルタイムで |
