...
nonisolated struct YAMLReplayExporter {  // swiftlint:disable:this type_body_length
  ...
  /// Default inter-event pacing (milliseconds) emitted for agent turns.
  /// Constant rather than derived from `TurnRecord.createdAt` diffs —
  /// persistence timestamps include streaming + BG-scheduling lag that
  /// do not reflect "natural simulation pace". Matches the spec §3.2
  /// example values.
  static let defaultTurnDelayMs = 1200
  /// Default inter-event pacing (milliseconds) for code-phase events.
  static let defaultCodePhaseDelayMs = 500
  ...
  private func renderYAML(_ input: Input) throws -> String {
    ...
    let estimatedDurationMs =
      totalTurns * Self.defaultTurnDelayMs
      + sortedEvents.count * Self.defaultCodePhaseDelayMs
  ...
  private func renderTurns(turns: [TurnRecord], phaseIndices: [Int]) -> String {
    ...
    for (idx, turn) in turns.enumerated() {
      ...
      lines.append("    delay_ms_before: \(Self.defaultTurnDelayMs)")
    }
    return lines.joined(separator: "\n")
  }
  ...
  private func renderCodePhaseEvents(
    ...
    for (idx, event) in events.enumerated() {
      ...
      lines.append("    delay_ms_before: \(Self.defaultCodePhaseDelayMs)")
    }
    return lines.joined(separator: "\n")
  }
