...
- **Dark mode (§2.9)** — **resolved 2026-07-31, no longer live debt.** All five of ADR-028 § "Rollout"'s removal gates are met and `Info.plist`'s `UIUserInterfaceStyle = Light` is gone, so the app follows the device appearance. The four-slice history, the derivation arms, and the load-bearing "1 unpaired (`headerMetaSubdued`, resolved as fixed in both appearances) vs 0 owing gate 1 an answer" distinction live in [ADR-028](decisions/ADR-028.md) and its INDEX summary — kept there rather than mirrored here, since this bullet went stale on every slice. 残件は `nightPage` の `ViewerPredictionSheet` 適用と、ゲートを閉じた QA より後に入った 4 面の再確認のみ — `docs/qa/dark-mode-qa.md` の再走リストが正本。Manual QA walkthrough: `docs/qa/dark-mode-qa.md`. Tracked in #1274 / #1284.
