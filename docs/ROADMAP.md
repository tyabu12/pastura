...
- **Dark mode (§2.9)** — post-v1.0 design debt, **not Phase 3 scope**, so it is not blocked by the Phase 3 gate. The pairing mechanism landed (ADR-028) and **40 light tokens** now resolve trait-based against their `night*` counterparts (the *light* alias is the dynamic side; `night*` stays static). Two of the five removal gates are met: raw-SwiftUI-colour tokenization (#1283 — gate 3; also added `inkOnAccent` and retired `Color.accentColor`) and the `nightSurface` double-mapping, resolved by deleting it (#1282 — gate 2). What remains is staged — settle the other **28** unpaired light tokens (visual sign-off per token), then real-device QA before `Info.plist`'s `UIUserInterfaceStyle = Light` can be removed. Note "settle", not "design": **27** of the 28 owe gate 1 a dark value, while `headerMetaSubdued` is already resolved as fixed in both appearances, which gate 1 accepts as an equal answer — so the two counts are not interchangeable. #1282 shipped **slice 1 of 4** (§2.6 alert family + §2.7 interactive states, 18 tokens) and #1313 **slice 2** (§2.4 meta L1–L4 + §2.12 GameHeader slots, 15 tokens); the remaining slices are the 17-colour avatar palette and the §2.1/§2.3/§2.8 remainder plus `inkOnAccent`. Sequence by **screen**, not token count: Home was already near-complete because its dominant tokens are all in the original paired 8; the alert-family / interactive-state surfaces (Settings, download progress) carried the bulk of the gap, which is why they went first. Avatar remains. Tracked in #1274.
