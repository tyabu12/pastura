...
**Two gaps no automated check covers** (mirror the § "Form A is a runtime
hazard" precedent):

- **Coverage is blind to the en plural.** `check_localization_coverage.py`'s
  `_validate_source_locale` reads `localizations.en.stringUnit` directly; a
  plural en block has no direct `stringUnit` (it lives under
  `variations.plural.*`), so it early-returns and never checks the `one`/`other`
  states. Confirm both are `"translated"` by hand / via the change-detector test.
- **Runtime firing needs device QA.** Build + coverage prove the structure, not
  that the variant fires. Put en n=1 → "1 record", n=2 → "2 records", ja
  unchanged in the PR body as a manual-QA item.

Test the structure (not runtime — ADR-009 forbids ViewInspector, and a
test-bundle `String(localized:)` resolve is bundle-ambiguous + tautological)
with a catalog-JSON change-detector. Canonical:
