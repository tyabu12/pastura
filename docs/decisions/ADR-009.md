...
5. Review-only screenshot capture (`ScreenshotTourTests` +
   `scripts/ui-tour.sh`) does **not** conflict with decision 4: it asserts
   nothing about rendered output, stores no reference image, and gates no
   CI (skipped via `-skip-testing` in `ci.yml`) — it exists solely so
   humans and agents can see the current UI during design review. The
   moment someone wants to *compare* those screenshots automatically,
   decision 4 applies in full.
