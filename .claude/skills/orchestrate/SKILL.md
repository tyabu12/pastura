...
2. If a plan comment is found:
   ...
   - Extract `Routing` from `## Metadata`. It records which label semantics the comment was written under, and it governs **the locus only** — which Step 3 branch a resumed item takes. Nothing else reads it; in particular the Coupling predicates do not, because they already degrade correctly (a pre-split comment carries no `🧠`). `v2` ⇒ current semantics: `🎭` dispatches to an Opus subagent, `🧠` is implemented in the session. **Absent, or any value this skill does not recognize** ⇒ **fail closed to legacy**: treat every `🎭` item in that comment as implemented in the main session, its pre-split meaning. An unrecognized value is treated exactly like an absent one — do not guess forward from a higher version number. Fail-closed is the safe direction: legacy locus is more expensive and never less reviewed, whereas guessing `v2` on an old comment would silently dispatch work the plan author expected to write themselves. **Fail-closed does not itself touch `SESSION_MODEL`**: a legacy comment bearing `🎭` is pinned to Opus by the Coupling re-check on the next line, and a legacy comment with no `🎭` keeps whatever `Session` it recorded — do not let fail-closed overwrite `SESSION_MODEL`, or every pre-v2 all-🎵 plan loses its Sonnet lever on resumption.
   - Derive `SLUG` from the branch name.
   - **Coupling re-check**: if the resumed plan contains any Opus-tier item (`🎭` or `🧠`) but `REVIEWER_MODEL=sonnet` **or** `SESSION_MODEL=sonnet` (e.g., from a post-plan Metadata edit that bypassed the Step 1 coupling rule), warn the user and offer to upgrade the affected model(s) to Opus before continuing — that is, before proceeding to Step 2 in the normal flow, or before the Step 4 review when all items are already complete. Reason: Opus-tier work must never be implemented or reviewed on Sonnet — `🧠` runs in the session itself, `🎭` dispatches to an Opus subagent, and both need an Opus reviewer. This predicate needs no version-conditioning: a plan comment written before the tier/locus split carries no `🧠`, so `any (🎭 or 🧠)` evaluates identically to the old `any 🎭` on those bodies. When `SESSION_MODEL` resolves to (or is upgraded to) `opus`, ensure the running session is actually on Opus (`/model opus`) before resuming implementation.
   - **All-Sonnet-tier re-check**: the Opus-tier predicate above (`🎭` or `🧠`) cannot fire on a plan whose every item is Sonnet-tier — `🎵`, `🎵 (tb)`, `🎵 (main)` in any mix — but 🎵 does not certify simplicity — Step 1.3's Q1 deferral also admits *specifiable but merely hard* items, marked `🎵 (tb)` at plan time. A resumed session has no conversation context to re-derive that classification itself — don't try. Instead, when the resumed plan is all 🎵 **and** `REVIEWER_MODEL=sonnet`: grep the plan comment for `🎵 (tb)`. Any hit ⇒ offer to upgrade `REVIEWER_MODEL` to Opus (the reviewer only — unlike the re-check above, which covers both models). **Fail closed on a zero-hit grep too**: a plan comment predating this marker carries no route information at all, and is indistinguishable by grep from a plan that legitimately used zero tie-breaker items — so treat "no `(tb)` found" as inconclusive, not confirmed-safe, and offer the same upgrade. `SESSION_MODEL=sonnet` stays — the cost lever survives an Opus reviewer.
...
3. Format the plan as a numbered checkbox list (each item = one planned commit).
   ...
   - **Q2 — locus, by specifiability.** Not "too hard for Sonnet?": difficulty or size alone does not pull an item into the main session. A subagent inherits none of this conversation, so ask: can you fill the Step 3 delegation prompt slots *now* without a new design decision — target file(s), plus an existing pattern to mirror or an acceptance condition? **Yes, even if hard → subagent.** No, or you can't tell whether the slots are fillable → **main session** (that specific uncertainty — not general unease). Also route to the main session when delegation prompt + verify overhead exceeds the implementation itself — single-line edits, short doc tweaks. **That disjunct cannot fire on a Q1-deferred item**: an item small enough for overhead to dominate is a "minor fix" or a "doc comment", which are 🎵 criteria, so Q1 matched it and did not defer. If you find yourself reaching for the overhead clause on a deferred item, re-run Q1 — the answer is Sonnet tier. **Q3's `deferred | main session` row rests on this sentence**: relax it and that row's stated reason goes false, and a fillable trivial item silently acquires the Opus tier again.
...
4. **Assign a reviewer model** for the PR as a whole (single choice, not per-item). This determines which model runs the `code-reviewer` subagent at Step 4. Criteria:
   ...
   **Sonnet reviewer acceptable** — the change must be strictly a subset of the 🎵 simple-item criteria above (existing pattern reuse, test-only changes following an existing pattern, type/error case additions, doc comments, minor fixes), AND none of the items match the Opus-required list. **Test that independently — a 🎵 label does not certify it.** Step 1.3's Q1 deferral also routes *specifiable but merely hard* items to the Sonnet tier — they carry `(tb)` — and those are not strictly simple: if the plan has one, the reviewer is Opus. `🎵 (main)` does not trigger this; it is a clean 🎵-criteria match that merely isn't worth a delegation prompt. Keep `Session: Sonnet` there — the cost lever survives an Opus reviewer. Concretely, the most common Sonnet-acceptable shapes are:
...
5. **Assign a session model** for the implementation phase (Step 3 onward). This is the model the orchestrator itself runs as — the user switches it manually via `/model` at G2 (or the combined single-commit gate), since the orchestrator cannot change its own session model. Derivation is **label-driven only** (unlike the reviewer, which also has path-based Opus triggers):
   ...
   - **Any plan item is Opus-tier (`🎭` or `🧠`)** → `Session`: **Opus**. Opus-tier work is judgment-heavy whether the orchestrator types it directly (`🧠`) or dispatches it (`🎭`) — and in both cases the orchestrator's own pre-commit spot-check runs at the session model, so that model must be Opus.

     `Session` stays keyed on the **tier**, not the locus. It is tempting to key it on `🧠` alone — "only `🧠` runs in the session, so only `🧠` needs an Opus session" — but the orchestrator still reviews and commits every `🎭` subagent's diff, and a Sonnet orchestrator must not be the pre-commit spot-check on an Opus-tier change. Do not simplify this predicate to `🧠`.
   - **Every item is Sonnet-tier** — `🎵`, `🎵 (tb)`, `🎵 (main)` in any mix → `Session`: **Sonnet** (recommended). The orchestrator's remaining work is dispatch, diff spot-check, commit, and PR mechanics, plus typing any `🎵 (main)` item itself. This is the primary cost lever: the long implementation tail runs at Sonnet rates. `🎵 (main)` belongs here deliberately — it is Sonnet-tier work that merely isn't worth a delegation prompt, so it must not drag the session up a tier.
...
**Resumed plans**: when Step 0 resolved `Routing` to legacy — absent or unrecognized — a `🎭` item takes the **🧠** branch instead, because pre-split `🎭` meant in-session. This table is the only site that reads that resolution; without this line the marker is emitted, parsed, and then ignored.
...
>
> **Add** — "End your report with one line: `OUTCOME: done | design-decision | scope-refusal | failed`, and state explicitly whether you left modified or new files in the worktree." Without it the four outcomes below are inferred from free prose, and they carry *opposite* stash instructions — mis-reading a design-decision stop as a failure stashes work this branch explicitly wants kept. It also makes the outcome-2 telemetry countable rather than impressionistic.
...
4. **Failed** — tried and could not finish. Take over via the 🎵 fallback path below, `git stash -u` included, but take its **`SESSION_MODEL=opus` arm**. Its Sonnet arm re-dispatches `claude-kit:implementer`, which here would answer a failed delegation with a second one — the mirror of the hazard flagged on that arm's sibling. That arm stays reachable on a 🎭-bearing plan because Step 0's Coupling re-check only *offers* the Opus upgrade: if the session is still Sonnet because the user declined, escalate to the user or to a high-effort general Opus subagent rather than re-dispatching.
