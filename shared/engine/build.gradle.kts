...
// ── Exported-header gates: shared reader ───────────────────────────────────
//
// Both gates below read the SAME generated header off the SAME single link.
// The path is shared so the two cannot drift apart silently.
val exportedHeaderPath =
...
// Configuration-cache note: this is a SCRIPT-level function, so a `doLast` that
// calls it captures the build-script object. The cache is off today (there is no
// `gradle.properties`), but turning it on will fail here first — hoist this into
// `buildSrc` at that point rather than sprinkling
// `notCompatibleWithConfigurationCache(...)` on the tasks.
...
val verifyEngineFrameworkSurface by tasks.registering {
    ...
    val headerFile = layout.buildDirectory.file(exportedHeaderPath)
...
// them, this fails red with "without an error: parameter" — safe direction, but
// read that message against the header before treating it as a real regression.
