...
file edit when you use Claude Code's PostToolUse hooks. The
`gh pr create` time CLAUDE.md-update reminder lives at
`scripts/hooks/check-claude-md-modified.sh` and is wired through
`.claude/settings.json`. The split between git pre-commit (commit-time
gates) and Claude Code hooks (edit-time + PR-creation reminders) is
deliberate — see #410 / memory `reference_claudecode_hook_matcher.md`
