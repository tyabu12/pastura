...
| `reason=no-grammar` | the turn had no `output:` schema, so `createSampler` returned before the DRY builder. **Zero in every battery cell today — an empirical fact about the bundled presets, not a structural guarantee.** Only `narrate` is Engine-fixed (`{ commentary }`); `reflect` / `whisper` are author-declared and merely validator-enforced, and `OutputSchema.from(phase:)` returns `nil` for any LLM phase whose `output:` block is absent or empty. `validateCanonicalFields` runs from `validateForCommit`, **not** from the `validate` the run path uses — so a scenario missing `output:` runs. Read a non-zero count as **a candidate scenario with a missing `output:` block** first (exactly what this skill feeds the battery), and only then as a new phase or handler that skipped the schema |
