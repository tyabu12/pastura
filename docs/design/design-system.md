...
- ⚠️ **上の免除は「測った地」にしか及ばない。** ≈ 3.3:1 は `screen-bg` 上の値で、`muted` は出荷している12の地（両外観 6 ずつ）で **2.136〜4.152** に散る。最悪は `--moss-soft` 上の 2.136、ダークで最もバーに近いのが `night-page` 上の 4.152（バーの 8% 下）。**ページ地で正当にアンビエントなラベルが、色付きの塗りの上では大きく下回っていても「§8 公認」に見える** — 地を変えたら測り直すこと。12組は `DesignTokensTests+MutedTranscript` が pin しており、**この行の幅は写しなので** `scripts/check-measurement-transcripts.py` がその pin の最小・最大と突き合わせている（#1488）。残りサイトのアプリ全体掃引は #1448
