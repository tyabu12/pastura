...
- **A reserved-but-unwritten ADR** (ADR-006 was one until 2026-09-08) must sit in `CLAUDE.md` as a **table row with the path in cell 1** carrying "reserved" / "not yet written": `consistency-audit`'s `load_reserved_adrs` parses that shape to suppress `dangling_adr` false positives and fails open if it changes. No ADR is reserved today; the row shape is what to reproduce for the next one.
