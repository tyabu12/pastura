...
- **User-facing String literals:** Any new user-facing English `String` literal — `Text("...")`, alert / toast / `errorMessage` assignments, accessibility labels — must be wrapped in `String(localized: "...")` so it lands in `Localizable.xcstrings` and gets a `ja` translation. CI's `localization-coverage` job (`scripts/check_localization_coverage.py`) fails when any catalog key lacks a non-empty `ja` value. The `#292` follow-up adds a SwiftLint rule to catch the common leak shape (`errorMessage = "..."` without wrap) at edit time.
