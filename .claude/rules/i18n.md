...
Workflow conventions for writing `String(localized:)` literals in **Swift source**. See `docs/i18n/leak-detection.md` for the broader leak-detection architecture (SwiftLint tripwire + audit script + CI gate).
...
**Distinct root cause from § "Form A is a runtime hazard at user-facing callsites" above**: there the key
