...
| `App/SimulationViewModel` suite Logger interpolations + BG identifiers (63 candidates, re-measured 2026-08-07; the prior 50 had drifted independently of this row's last edit) | `Pastura/Pastura/App/SimulationViewModel.swift`, `Pastura/Pastura/App/SimulationViewModel+Background.swift`, `Pastura/Pastura/App/BackgroundSimulationManager.swift` | **Permanent** — ~43 `os.Logger` `info` / `notice` / `error` interpolations annotated `privacy: .public` per CLAUDE.md Logger-privacy rule (lifecycle / scenePhase / BG-task diagnostics like `"scenePhase=.active enter: isRunning=%arg, ..."`, `"BG task activation: isRunning=%arg, ..."`, `"committed agent=%arg totalAttempts=%arg"`); 5 Logger subsystem / category strings (`app.pastura.Pastura` × 2 + `SimulationVM`, `StreamingDiag`, `BGSimManager`); 2 BGTaskScheduler identifier fragments — the identifier is derived from the bundle ID (the Debug configuration suffixes it), so the audit sees `app.pastura.Pastura` as the `??` fallback plus `%arg.simulation-continuation` as the interpolation template, rather than one literal; 2 `cancelSimulation(caller:)` debug tags (`switchToCPUInference-error`, `switchToGPUInference-error`). Diagnostic-only, never user-facing UI strings. Per #340 slice-7 decision. |
