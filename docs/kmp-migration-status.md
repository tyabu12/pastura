...
- **Stage 4** (parity harness): 🔄 in progress — **both parity rungs are live**; the residue is
  ...
  Of the eight fixtures the six nominal ones carry the real-scenario parity claim (every handler
  is now witnessed by at least one fixture — "witnessed" here means replayed through a golden,
  distinct from the machine-checked *ported* checklist above); the two controls exist so the
  ledger's own mechanisms stay provably reachable, with
