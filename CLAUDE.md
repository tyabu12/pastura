...
- **Automated hooks** — split by gate type. Activate the git side once per clone with `./scripts/setup.sh`.
  ...
  - **Opt-in, per-user — deliberately NOT wired here**: `scripts/prune-stale-worktrees.sh` removes stale auto-named worktrees left behind by unattended routines (nobody answers `EnterWorktree`'s exit prompt on a nightly run, so residue accumulates). It is destructive and the routines motivating it are per-machine Desktop config, so wire it in your own untracked `.claude/settings.local.json` as a `SessionStart` hook rather than in the tracked `settings.json` — dry-run (the default) until its log reads right, then `--apply`. Predicate, measured safety layering and how to tune the disposable set: the script's header.
