...
**Its router is read as an optional `@Environment(AppRouter.self)`, and must stay optional.** iOS 26 can size a toolbar item from inside the push transition (`UIKitBarItemHost.initializeSize()` via `BarAppearanceBridge.didMoveToWindow`), reaching the item's environment read before `TabNavigationStack`'s `.environment(router)` propagates to that separate view graph; the non-optional form then trips the `EnvironmentValues.subscript` assertion and terminates the app (TestFlight 1.3 (888) / iOS 26.6, #1683 — one report, never reproduced). Nothing in the build or the test suite catches a regression here: ADR-009 rules out render tests, and the failure needs a real push transition. The nil arm logs and calls `dismiss()` rather than returning silently, so a router that never arrives shows up in the log instead of as a dead back button (the `dismiss()` recovery is best-effort — the same incomplete environment can hand back the default no-op `DismissAction`).

**Generalized: a router read from inside a `ToolbarItem` must be optional; body-level reads stay non-optional.** A toolbar item is hosted in its own view graph and can be sized before the pushed environment reaches it, which is the whole hazard above; a view's own `body` is not, so the six body-level `@Environment(AppRouter.self)` reads correctly stay non-optional and keep failing loudly on a genuine wiring mistake.
