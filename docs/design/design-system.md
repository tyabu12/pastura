...
**セクションラベルは `--ink-2`。自前ヘッダーもこの表どおりに揃っている（#1485）。** `PasturaSection` のヘッダーと、それを通らない手書きヘッダー2つ（`HomeView.scenariosSectionHeader` / `SettingsView+Models.modelsHeader`）は `--muted` で描かれ表と食い違っていた。#1448 が「コードを表に寄せる」と決め — §2.2 が役割の規範的な記述であること、#1298 が既に先例を作っていること、監査で「表のほうが誤り」とする論拠が出なかったこと（導出は `muted-application-audit` §6.3）— #1485 で適用した。**5画面**（ホーム / 設定 / 観察履歴 / ScenarioDetail / GalleryScenarioDetail）にまたがる視覚変更なので ADR-028 gate 4/5 の実機 QA を伴う。掃引対象を数え直すときの正は**呼び出し形**の grep — `rg -l 'PasturaSection\s*[({]' Pastura/Pastura/Views/`。型名だけの grep は `PasturaSectionStyle` の言及も拾って広く出るし、`\(` だけの grep は無題カードの末尾クロージャ形を落とす。**件数はここに写さない**（`muted-application-audit` §6.3 が列挙を持つ。消費側の数は「決定が変わらないまま動く」在庫で、`adr-writing.md` §4 の写し禁止に当たる）— ただし consumer ファイル数 / ヘッダーを実際に描くファイル数 / 画面数は**別の量**で、混ぜると規模を逆向きに誤る。上の5は QA 規模を決める画面数のほうで、§6.3 が3つを分けて持つ。**残るのはシステム `Form` / `List` の section ヘッダーだが、これは同じ問題ではない。** どのトークンも読んでおらず、動かすには「Pastura は system `Form` の chrome をトークン化するのか」という未決の判断が要る（21サイト / 9ファイル。列挙の形と根拠は §6.3、追跡は台帳 §7 の B6）。#1298 が `ScenarioEditorView` の Personas / Phases だけを揃えたのは、隣に `--muted` のカウントが並び、明度がほぼ同じで色相だけ違う組み合わせになっていたためで、system `Form` 一般をトークン化する先例ではない。
