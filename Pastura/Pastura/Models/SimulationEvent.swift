...
nonisolated public enum SimulationEvent: Sendable, Equatable {
  ...
  ///
  /// A single LLM inference per round makes a commentator persona narrate the
  /// round's highlight. Distinct from `.summary` (deterministic template /
  /// scoring text) in two load-bearing ways:
  ///
  /// - **It is free LLM prose about the agents**, so consumers MUST run it
  ///   through `ContentFilter` before display — unlike `.summary`, whose
  ///   deterministic text the live path renders raw (see
  ///   `SimulationViewModel.handleOutputEvent` / `ReplayViewModel`). App Store
  ///   content-safety (ADR-005) depends on this.
  /// - It carries no agent attribution: the narrator is not a participant, so
  ///   it never appears in votes, scores, or the scoreboard.
  ///
  /// Persisted via ``CodePhaseEventPayload/narration(text:)`` so it survives in
  /// past-results, export, and replay.
