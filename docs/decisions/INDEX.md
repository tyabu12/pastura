...
Multi-platform strategy — Accepted (GO) on #220 KMP spike; §9 GO/NO-GO synthesis (H5/H7 distribution-verification deferred at acceptance, discharged 2026-09-05 — §11; §12 Amendment 2026-09-06 — ADR-023 Decision 6 (iii) discharged on the `v1.3+888` soak, ADR-004 status unchanged; **§13 Amendment 2026-09-07** — ADR-023 S5-5 landed and Decision 6 fired, so **§9.7 closes**: both Phase 3.0 code-merge landmines were already discharged and the remaining spike close-out landed with S5-5 (gate spike retired, B′ guard re-homed to `scripts/kmp/check-b-prime-isolation.sh`, `macosArm64` parity rung untouched); §11's H7-probe deletion deadline is discharged; status still `Accepted — GO`)
...
never derived by reflection; Decision 6: every listed disclosure surface
updates in the same release as the feature or any provider change, and the
on-device claim is scoped to *simulation*. Still open: the proxy /
maintainer-key model waits on the marketplace backend (Revisit trigger).
...
KMP Engine migration architecture (Phase 3.0) — run-path Engine port to `commonMain` with callback-only K/N boundaries (event: `SharedEngineRunner` adapter; inference: `LLMBackend`/`StreamCallbacks`; no suspend/Flow crosses, `SuspendController` never crosses); Stage-2 two-boundary vertical slice = GO/NO-GO gate; Stage-4 cross-language parity harness (JVM per-PR + macosArm64 nightly, mechanism contract); D1 iOS switch A-with-gate (both-rung parity + H7 symbolication + TestFlight soak), D2 Data stays Swift/GRDB (SQLDelight revisit at Android scoping), D3 ~4–5 wk + 1 wk (**superseded** — inherits a one-day-early sizing basis; see §12 condition 3 for what must be recorded before Stage 3 is scheduled); serialization path + LLM backends stay Swift. **§4 revised 2026-07-19** — an inventory of `Engine/**`+`LLM/**` found 3,449 lines in neither list (an undecided disposition, four mechanisms that post-dated the section, and 1,756 lines the port/stay pair could not express); §4 now carries four dispositions (`PORT`/`STAY`/`REPLACED`/`FOLDED`; a fifth, `SPLIT`, since §13), two inheritance rules (sibling extensions, `LLMService` conformers) and a **deliberate absence of any catch-all** — a file no rule reaches is `UNDECIDED`, a failure state rather than a default, because a rule that always answers cannot raise an alarm and the four mechanisms that opened the gap were standalone `Engine/` types no inheritance rule could reach; coverage invariant left explicitly unenforced pending #1191 (enforced since #1197, 2026-07-21: ledger `shared/adr-023-port-ledger.tsv` + `check-adr023-port-coverage.py`). The ADR-024 linter folds into Load+validate (shared consumers; its `.error` findings gate the run inside the ported `preflightGate`); port surface is now 8,044 lines Engine+LLM — not to be confused with §12's Engine-only 8,088; ADR-021/-022 numbers held by #992 / PR #997. **§12 Amendment 2026-07-18 — Stage-2 gate verdict: GO**, with four conditions: (1) align `OutputSchema` at the head of Stage 3, Kotlin moving to Swift's shape — surface includes `from()`'s CHOOSE construction branch, the `LLMBackend.schema` passthrough (which follows the type with no edit — not a second DTO), and Swift's own ADR-021-stale rationale comment; shipped in #1193, where the Swift↔Kotlin tag form was ruled absorbed test-side (no production `KSerializer`, since nothing JSON-crosses `OutputSchema`); (2) every type mirrored into `shared/**` lands with golden-parity coverage in the same PR (the `OutputSchema` mirror was born diverged, not rotted — so the lesson is coverage-at-landing, not cadence; whether the JVM rung also moves earlier is left open); (3) replace §10's velocity-taper assumption with a pinned detector — trailing-10-day commit count over the **port surface** (a derivation from §4, not a frozen list; LLM/ wholesale would count STAY backend churn, while Engine-only missed the LLM port surface — 1 commit in the calibration window, the whole 16→17 unfiltered delta), ≤5 schedules / >5 means wait or dual-land; baseline re-measured to 17 unfiltered / 16 filtered, superseding the Engine-only 16/15 — the two 16s mean different things — with the reading and either the re-estimate or the dual-landing decision recorded on #501 before the first Stage-3 PR, priced against **both** 8,044 production lines and ~1,658 lines of test siblings owed Kotlin counterparts under condition 4; the §4 amendment lands first or the reading is stale on arrival. Stage-3 precondition surfaced by it: `VoteTally` needs Models `RankingOrder` in the freeze-by-design `shared/models`, and condition 2 governs how a mirror lands, not whether a frozen module accepts one; (4) "module done" = a `commonTest` sibling shown to go red under perturbation of the ported code, not merely one that exists. §6 Stage 3 carries the gate; an abandonment trigger (handler bodies needing Swift-side restructuring to stay expressible) lets the stage stop — whether "handler body" should widen to any ported module body is tracked in #1191, not settled here. Accepted on faith and routed to later stages: macOS↔iOS K/N equivalence, K/N GC under device memory pressure (the largest uncovered risk; D1 + H7 backstop it), the Swift-decodes-Kotlin direction, H5/H7 distribution. **§13 Amendment 2026-08-22 — `SPLIT`** (one Swift file → N Kotlin files, the direction `FOLDED` could not express; #1465): a terminal disposition, not a progress state — its `kotlin_target` lists every Kotlin file the content landed in as repo-relative `shared/**` `.kt` paths the checker verifies are tracked, and the gate's triggers widen to `shared/**` `.kt` so a target rename fires it; `ScenarioLoader` is the instance (`estimateInferenceCount` → `InferenceEstimator.kt`, remainder outstanding). **§16 Amendment 2026-09-07 — S5-5 landed: Decision 6 fired, Stage 5 closed** (#1685): the Kotlin engine is the shipping engine for every fresh run and the Swift `SimulationRunner` stays for `resume()` alone, which Kotlin exports no entry point for; the S5-3 H7 probe and the S5-4 opt-in switch are deleted in full (including `PasturaApp.clearSharedEngineOptInOffSandbox()`, whose survival would have wiped the new default on every production launch), discharging ADR-004 §11's pre-submission deadline, and ADR-004 §9.7 closes (ADR-004 §13). The **Stage-2 gate spike is retired** — the iOS app is the real consumer and `PasturaTests/App/KMP/` covers what the spike's probes did — with every non-parity asset re-homed first (B′ guard → `scripts/kmp/check-b-prime-isolation.sh` under the ruleset-frozen `kmp-gate-isolation` name, `suspendcontroller-drift` job and its ruleset entry deleted, nightly Pattern 7 probe staging the app-tree slice), the Stage-2 bench retiring with it and the `macosArm64` parity rung untouched; **Decision B′ is superseded as a host decision while its substance stands** — no per-PR lane acquires an XCFramework dependency, now enforced by the re-homed script rather than from inside the package it guards. Two **in-place factual corrections** to §6's S5-5 bullet: "delete the Swift run-path the ledger marks `REPLACED`" described a deletion that does not exist (the `REPLACED` rows are `LLM/` types both engines share via `LLMServiceBackend`, and `resume()` plus the Stage-4 harness instantiate `SimulationRunner` directly, so no `Engine/` file is deletable), and "the Swift runner becomes the sole path" had the direction backwards. Accepted residuals: the YAML-less run branch kept as a **test seam** (discharged 2026-09-08, §17), made loud (`assertionFailure` off the test harness + an always-on `.error` log) and tracked by [#1687](https://github.com/tyabu12/pastura/issues/1687); and **no Swift fallback** for a YAML the Swift loader accepts at open time but the Kotlin loader rejects — it surfaces as `.scenarioValidationFailed` at run time, guarded by the differential `LoaderAcceptanceParityTests`. The S5-4 `TurnOutput.rawText` residual is closed by porting the field as a body-declared `@Transient var` — outside `equals` / `copy` / the wire shape, so `TurnRecord.rawOutput` populates on the Kotlin path with no GRDB migration. **§17 Amendment 2026-09-08 — test-seam residual discharged** (#1687): `yamlDefinition:` is required on `SimulationViewModel.run`, the YAML-less branch is deleted and the Swift runner serves `resume()` alone; the migration fixed `ScenarioSerializer`'s empty-phase-list output (`phases: []`) and surfaced that ADR-021 D3 systemic classification does not cross the K/N boundary — tracked by #1689, not accepted; Pattern-6 audit re-checked, unchanged (Status: Accepted; #501)
