...
- ⚠️ **上の免除は「測った地」にしか及ばない。** ≈ 3.3:1 は `screen-bg` 上の値で、`muted` は出荷している12の地（両外観 6 ずつ）で **2.136〜4.152** に散る。最悪は `--moss-soft` 上の 2.136、ダークで最もバーに近いのが `night-page` 上の 4.152（バーの 8% 下）。**ページ地で正当にアンビエントなラベルが、色付きの塗りの上では大きく下回っていても「§8 公認」に見える** — 地を変えたら測り直すこと。12組は `DesignTokensTests+MutedTranscript` が pin しており、**この行の幅は写しなので** `scripts/check-measurement-transcripts.py` がその pin の最小・最大と突き合わせている（#1488）。**突き合わせの対象外にある写しも野放しではない** — 同じチェッカーがファイル別に行数・相異なる値の数・ダイジェストを宣言と照合するので、この図をどこかへ写すと CI が落ちる（#1496、CI の `shell-tests` のみ）。落ちたら分類の判断が要るという意味であって、欠陥とは限らない。縛れるのは走査対象の拡張子・pin と同じ3桁表記、そして Swift ではコメントの**中**にある図に限る（実行される行の pin は guard に分類され、台帳に入らない） — 導出は ADR-028 § Amendment 2026-08-20 (#1496)。残りサイトのアプリ全体掃引は #1448
