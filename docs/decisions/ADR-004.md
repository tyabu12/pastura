...
> **Status:** Accepted — GO (KMP Models spike #220; H5/H7 distribution-verification discharged 2026-09-05 on `v1.3+886` — §11; the §9.3 unmeasured Tier-2 gates stay non-blocking watches)
...
`FeatureFlags.sharedEngineEnabled` on for the whole cycle. All four bundled
presets completed on the Kotlin engine — `target_score_race`, `word_wolf`,
`last_fable`, `prisoners_dilemma`, between them covering `conditional`,
`event_inject`, and the ADR-027 `pairwise_payoff` scoring path. Pause→resume and
