...
| `docs/decisions/ADR-022.md`           | Scenario semantic lint layer — `ScenarioSemanticLinter` (Engine) fires silent-no-op DSL traps at load time as findings (error blocks commit/run, warning never blocks; rule catalog R1–R17; linter-owned `PlaceholderAvailability` map absorbs #920's model; `pastura-harness lint` batch gate, 46 shipped YAMLs zero-FP; warn-first promotion policy for post-v1 error rules) (Status: Accepted; #994) |
