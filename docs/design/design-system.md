...
- ⚠️ **静的な比が出ない地でも routing は決まる。ただし「測れない」を最初の答えにしないこと。** マテリアル（`.ultraThinMaterial` / `.regularMaterial`）、半透明の上にさらに半透明が重なった層、Pastura トークンを持たないシステムのシート地 — この 3 種は真の地が実行時に決まる。**それでも**地を所有するファミリは読めるので、置き換え先の既定は上のバレットのまま: 中立なら `--ink-2`、インク系の半透明ウォッシュなら `--ink-on-wash`、苔系なら `--moss-on-wash`。根拠の付け方には**優先順位がある**:
  ...
  2. **それも書けないときに限り、方向で論証する。** 置き換え先が light で `--muted` より暗く、dark で `nightMuted` より明るいこと。⚠️ **これだけでは足りない** — 「暗くすれば比が上がる」は**地が `--muted` より明るいときにしか成り立たない**。地が両方の前景より暗いと逆転し、repoint はコントラストを**下げる**（`--muted` のほうが地から遠くなるため）。反例は `DesignTokensTests+MutedTranscript` の `directionArgumentInvertsOnAGroundDarkerThanMuted` が構成して pin しているので、比はここに写さない。だから方向論証は「地の明度が light では `--muted` より上、dark では `nightMuted` より下だと**言い切れる**場合」に限って使うこと。言い切れないなら 1 に戻るか、repoint 自体を見送る。使ったサイトは **why-comment に「方向論証」と明記し、比は書かない**（無い）。ガードは fixture ではなく `MutedSweepLedgerTests` の**シンボル固定**。先例: #1448 batch 1 の `DLCompleteOverlay`（マテリアルのみで下限が無い）、batch 3 の `ScoreboardSheet` の 2 行（システムのシート地）。
