...
**UI-layer traps live in `i18n-ui.md`** — plurals, the SwiftUI `LocalizedStringKey` traps, `#if DEBUG` extraction, `.accessibilityLabel` audit triage. Scoped to the UI paths + the catalog, so a non-UI session does **not** load it: every reference to it below is informational, never a dependency. Which file owns a section is decided by `docs/i18n/leak-detection.md` § "Rule-file layering".
